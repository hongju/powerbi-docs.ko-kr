...
저장소 모드를 설정하면 많은 이점이 제공됩니다. 모델에서 각 테이블의 저장소 모드를 개별적으로 설정할 수 있습니다. 이 작업을 수행하면 다음과 같은 이점을 제공하는 단일 데이터 집합을 사용할 수 있습니다.
...
* **쓰기 저장**: 쓰기 저장을 사용하면 비즈니스 사용자가 셀 값을 변경하여 what-if 시나리오를 살펴볼 수 있습니다. 사용자 지정 응용 프로그램이 변경 내용을 데이터 원본에 적용할 수 있습니다. 캐시되지 않은 테이블이 즉시 변경 내용을 표시하므로 효과를 즉시 분석할 수 있습니다.
...
* **가져오기**: 값을 **가져오기**로 설정하면 가져온 테이블이 캐시됩니다. 가져오기 테이블의 데이터를 반환하는 Power BI 데이터 집합에 제출된 쿼리는 캐시된 데이터에서만 수행할 수 있습니다.

* **DirectQuery**: 이 설정을 사용하면 DirectQuery 테이블이 캐시되지 않습니다. Power BI 데이터 집합을 제출하는 쿼리(예: DAX(Data Analysis Expressions) 쿼리) 및 DirectQuery 테이블에서 데이터를 반환하는 쿼리를 수행하려면 데이터 원본에 대한 주문형 쿼리를 실행해야 합니다. 데이터 원본에 제출하는 쿼리는 해당 데이터 원본의 쿼리 언어(예: SQL)를 사용합니다.
...
데이터 집합에 있는 하나 이상 테이블의 저장소 모드가 **가져오기** 또는 **이중**으로 설정된 경우 **데이터 보기** 탭이 표시됩니다.
