...
앱은 새로운 기능과 역량을 사용하여 정기적으로 업데이트됩니다. 최신 버전을 실행 중인지 확인 합니다. 이전 버전의 앱이 이미 설치되어 있는 경우 앱에서 삭제한 다음, CTRL+F5를 눌러 새로 고치는 것이 가장 좋습니다. 
...
이제 앱을 설치했으므로 조직의 용량에 대한 메트릭을 볼 수 있습니다. 앱은 메트릭 요약을 사용 하 여 대시보드 및 자세한 메트릭 보고서를 제공합니다.
...
| 총 새로 고침 수 | 지난 7일간 총 새로 고침 수입니다. |
| 새로 고침 안정성(%) | 지난 7일간 성공한 새로 고침 수를 총 새로 고침 수로 나눈 값입니다. |
| CPU 최대 (%)| 지난 7 일 동안에서 AI 워크 로드에 따라 최대 CPU 사용입니다. |
| 메모리 최대값 (GB) | 최대 메모리 사용 AI 워크 로드에서 지난 7 일입니다.|
| 최대 대기 시간을 (밀리초)를 새로 고칩니다. | 새로 고침을 시작 하기 전에 최대 양입니다. |
| 새로 고침 평균 대기 시간 (MS)| 새로 고침 시작 전에 대기하는 평균 시간입니다. |
| 새로 고침 최대 기간 (MS) | 최대 크기 전체 새로 고침 시간입니다. |
| 새로 고침 평균 기간 (밀리초)| 새로 고침 완료에 걸리는 평균 시간입니다. |
| | |
...
[**AI** ](#ai) -여 용량에 사용 되는 AI 함수의 상태에서 자세한 메트릭을 제공 합니다.
...
| 보고서 섹션 | 메트릭 |
| --- | --- |
| 새로 고침 |  총 개수: 각 데이터 세트의 총 새로 고침 횟수입니다.<br>  안정성: 각 데이터 집합에 대해 완료 하는 새로 고침의 비율입니다.<br>  평균 대기 시간: 데이터 세트 새로 고침의 예약 시간과 시작 시간 사이의 평균 지연 시간(분)입니다.<br>  최대 대기 시간: 데이터 세트에 대한 최대 대기 시간(분)입니다.<br>  평균 기간: 데이터 세트의 평균 새로 고침 기간(분)입니다.<br>  최대 기간: 데이터 세트에 대해 가장 오래 실행된 새로 고침 기간(분)입니다. |
...
| **보고서 섹션** | **메트릭** |
| --- | --- |
| 새로 고침 |  전체: 각 데이터 흐름에 대한 총 새로 고침 수입니다.<br>  안정성: 각 데이터 흐름에 대 한 완료 된 새로 고침의 비율입니다.<br>  평균 대기 시간: 데이터 흐름 새로 고침의 예약 시간과 시작 시간 사이의 평균 지연 시간(분)입니다.<br>  최대 대기 시간: 데이터 흐름에 대한 최대 대기 시간(분)입니다.<br>  평균 기간: 데이터 흐름에 대한 평균 새로 고침 기간(분)입니다.<br>  최대 기간: 데이터 흐름에 대해 가장 오래 실행된 새로 고침 기간(분)입니다. |
...
| **보고서 섹션** | **메트릭** |
| --- | --- |
| 전체 사용량 | 총 개수: 작업 영역 또는 데이터 흐름에서 AI 함수 개수입니다. <br> 안정성: 새로 고침 완료 비율입니다.<br> 평균 대기 시간: 평균 지연 시간 예약 된 시간 사이의 시간 (밀리초)의 새로 고침을 시작 합니다.<br> 최대 대기 시간: 최대 대기 시간 (밀리초)입니다.<br> 평균 기간: 평균 기간 (밀리초) 새로 고침입니다.<br> 최대 기간: 기간 오래 실행 새로 고침 시간 (밀리초)입니다.<br> 평균 입력 크기: 평균 입력 크기 (바이트)를 새로 고치는 데이터 흐름에서 실행 하는 AI 함수에 대 한 합니다.<br> Avg 출력 크기: 평균 크기 (바이트)를 새로 고치는 데이터 흐름에서 실행 하는 AI 함수에 대 한 출력입니다. |
| 평균 기간별 상위 5 개 AI 함수 | 가장 긴 평균 새로 고침 시간 (밀리초) 사용 하 여 다섯 가지 함수입니다. |
| 평균 입력된 크기에 따라 상위 5 개 AI 함수 | 가장 큰 평균 데이터를 사용 하 여 다섯 가지 함수 크기 (바이트) 입력 합니다. |
| 시간별 새로 고침 수 및 메모리 사용량 | UTC 시간으로 보고되고 한 시간 버킷으로 분할된 성공, 실패 및 메모리 사용량입니다. |
| 시간별 평균 지속 시간 | 새로 고침을 1 시간 버킷으로 분할의 평균 지속 시간 UTC 시간에 보고 합니다. |
| | |
...
1. **액세스 제어(IAM)** 를 클릭하고 판독기 역할에 **Power BI Premium** 앱을 추가합니다. 앱 이름으로 찾을 수 없는 경우 추가할 수 있습니다도 클라이언트 식별자: `cb4dc29f-0bf4-402a-8b30-7511498ed654`합니다.
