...
**데이터 집합** 탭은 앱의 대량 메트릭을 제공합니다. 탭 맨 위에 있는 단추를 사용하여 **요약**, **새로 고침**, **쿼리 기간**, **쿼리 대기** 및 **데이터 집합** 영역으로 이동합니다.
...
**쿼리 기간** 영역에는 총 쿼리 실행 횟수와 평균/최대 기간(밀리초)이 나열됩니다. 이 데이터는 지난 7일 동안 데이터 집합, 작업 영역 및 시간별 버킷을 기준으로 분할됩니다. 맨 아래 차트에는 쿼리 수와 평균 기간(밀리초) 및 현지 시간으로 보고된 한 시간 버킷으로 분할된 메모리 사용량(GB)이 표시됩니다.

오른쪽 위 차트에는 쿼리 기간 분포 히스토그램이 표시됩니다. 히스토그램은 보고된 쿼리 기간(밀리초)을 기준으로 <= 30ms, 30-100ms, 100-300ms, 300ms-1초, 1초-3초, 3초-10초, 10초-30초 및 > 30초 간격 범주로 버킷 구성됩니다.

오른쪽 아래 차트에는 쿼리 완료에 든 평균 쿼리 기간에 따른 상위 5개 데이터 집합이 나열됩니다.

쿼리 기간이 길고 대기 시간이 길면 조회 지속 기간 및 장기 대기 시간은 용량이 과도하게 실행되고 있음을 나타냅니다. 또한 단일 데이터 집합에 문제가 있으며 추가 조사가 필요하다는 것을 의미할 수도 있습니다.

#### <a name="query-waits-area"></a>쿼리 대기 영역

**쿼리 대기** 영역에는 실행된 총 쿼리 수, 실시간 쿼리/직접 쿼리의 총 쿼리 대기 횟수 및 보고된 평균/최대 대기 시간(밀리초)이 나열됩니다. 이 데이터는 지난 7일 동안 데이터 집합, 작업 영역 및 시간별 버킷을 기준으로 분할됩니다. 맨 아래 차트에는 쿼리 대기 수와 평균 대기 시간(밀리초) 및 현지 시간으로 보고된 한 시간 버킷으로 분할된 메모리 사용량(GB)이 표시됩니다.

오른쪽 위 차트에는 쿼리 대기 시간 분포 히스토그램이 표시됩니다. 히스토그램은 보고된 쿼리 기간(밀리초)을 기준으로 <= 50ms, 50-100ms, 100-200ms, 200-400ms, 400ms-1초, 1초-5초 및 > 5초 간격 범주로 버킷 구성됩니다.

오른쪽 아래 차트에는 쿼리 시작에 든 평균 대기 시간에 따른 상위 5개 데이터 집합이 나열됩니다.
