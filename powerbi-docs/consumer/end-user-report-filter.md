...
이 예제에서는 두 개의 필터가 있는 시각적 개체를 선택했습니다. 보고서 페이지에는 **이 페이지의 필터** 머리글 아래에 나열된 필터도 있습니다. 또한 전체 보고서는 **날짜**에 대한 필터가 있습니다.
...
1. **분기**에 대해 슬라이서가 적용되었습니다.

1. **지역**은 이 보고서 페이지에 적용되는 필터이며,

1. **isVanArsdel** 및 **연도**는 이 시각적 개체에 적용되는 필터입니다.
...
**(All)** 로 설정하려는 필터가 하나뿐인 경우 지우개 아이콘 ![지우개 아이콘의 스크린샷](media/end-user-report-filter/power-bi-eraser-icon.png)을 선택하여 취소합니다. 필터 이름 옆에 있는
