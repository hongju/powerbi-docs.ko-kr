...
**drill up, drill down, drillthrough (드릴업, 드릴다운, 드릴스루)**
...
**environment (환경)** [Power BI Desktop, Power BI Mobile 등] Power BI 환경을 사용합니다. 다중 테넌트 고객의 경우 이 용어를 사용하여 연결할 테넌트 선택을 나타냅니다.
...
**focus mode (포커스 모드)** [Power BI 서비스]

**Free account (무료 계정)** 이 용어 모음의 계정 참조

**full screen, full-screen mode (전체 화면, 전체 화면 모드)** [Power BI 서비스]
...
**insights (인사이트)** 인사이트 기계 학습 알고리즘을 실행하여 수행하는 자동 데이터 분석입니다.
...
**On-premises data gateway (온-프레미스 데이터 게이트웨이)**
...
**page (페이지)** 보고서에는 하나 이상의 페이지가 있습니다. 보고서 캔버스 아래쪽의 각 탭은 페이지를 나타냅니다.
