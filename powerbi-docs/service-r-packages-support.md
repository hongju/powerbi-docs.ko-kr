...
* 현재 서비스에서는 보안 및 개인 정보 보호를 위해 World-Wide Web(예: RgoogleMaps)에 클라이언트 서버 쿼리를 제공하는 R 패키지를 지원하지 않습니다. 이러한 시도에 대한 네트워킹이 차단됩니다. 지원되거나 지원되지 않는 R 패키지 목록은 다음 섹션을 참조하세요.
...
| 수식 |1.2-1 |https://cran.r-project.org/web/packages/Formula/index.html |
...
| 행렬 |1.2-4 |https://cran.r-project.org/web/packages/Matrix/index.html |
...
## <a name="r-scripts-that-are-not-supported-in-power-bi"></a>Power BI에서 지원되지 않는 R 스크립트
