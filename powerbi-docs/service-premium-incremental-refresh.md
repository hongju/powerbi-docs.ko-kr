...
새로 고침 작업을 위해 쿼리가 제출될 때 필터가 소스 시스템으로 푸시되는 것이 중요합니다. 즉, 데이터 원본이 “쿼리 접기”를 지원해야 합니다. 각 데이터 원본에 대해 다양한 수준의 쿼리 접기가 지원되는 경우 원본 쿼리에 필터 논리가 포함되어 있는지 확인하는 것이 좋습니다. 필터 논리가 포함되어 있지 않으면 각 쿼리에서 원본의 모든 데이터를 요청하므로, 증분식 새로 고침 개체를 사용할 수 없게 됩니다.
 
필터는 데이터를 Power BI 서비스의 범위로 분할하는 데 사용되며, 필터링된 날짜 열의 업데이트를 지원하도록 설계되지는 않았습니다. 업데이트는 업데이트가 아닌 삽입 및 삭제로 해석됩니다. 삭제가 기록 범위에서 발생하고 증분 범위가 아닌 경우에는 선택되지 않습니다.
