...
- **새로 고침이 더 빠릅니다.** 변경된 데이터만 새로 고칠 필요가 있습니다. 예를 들어 10년 데이터 세트의 마지막 5일만 새로 고칩니다.
...
잠재적으로 수십억 개의 행이 있는 큰 데이터 세트는 일반적으로 사용자의 데스크톱 PC에서 사용할 수 있는 리소스에서 제한하므로 Power BI Desktop에 적합하지 않을 수 있습니다. 따라서 이러한 데이터 세트는 일반적으로 가져올 때 Power BI Desktop에 맞게 필터링됩니다. 이는 증분 새로 고침을 사용하는지 여부에 관계없이 계속됩니다.

#### <a name="rangestart-and-rangeend-parameters"></a>RangeStart 및 RangeEnd 매개 변수

Power BI 서비스에서 증분 새로 고침을 사용하려면 예약된 **RangeStart** 및 **RangeEnd**(대/소문자 구분 이름)이 포함된 파워 쿼리 날짜/시간 매개 변수를 사용하여 필터링을 수행해야 합니다.

일단 게시되면 Power BI 서비스에서 매개 변수 값이 자동으로 재정의됩니다. 따라서 서비스의 데이터 세트 설정에서 이를 설정할 필요가 없습니다.

새로 고침 작업을 위해 쿼리가 제출될 때 필터가 소스 시스템으로 푸시되는 것이 중요합니다. 필터링을 아래로 푸시하려면 데이터 원본이 “쿼리 접기”를 지원해야 합니다. SQL 쿼리를 지원하는 대부분의 데이터 원본은 쿼리 접기를 지원합니다. 플랫 파일, BLOB, 웹 및 OData 피드와 같은 데이터 원본은 일반적으로 그렇지 않습니다. 각 데이터 원본에 대해 다양한 수준의 쿼리 접기가 지원되는 경우 원본 쿼리에 필터 논리가 포함되어 있는지 확인하는 것이 좋습니다. 데이터 원본 백 엔드에서 필터를 지원하지 않을 경우 아래로 푸시될 수 없습니다. 이 경우 매시업 엔진이 로컬에서 필터를 보정하고 적용하며, 데이터 원본에서 전체 데이터 세트를 검색해야 할 수 있습니다. 따라서 Power BI 서비스 또는 온-프레미스 데이터 게이트웨이에서 사용할 경우 증분 새로 고침 속도가 매우 느려질 수 있고, 프로세스가 사용할 리소스가 부족해질 수 있습니다.

필터는 데이터를 Power BI 서비스의 범위로 분할하는 데 사용되며, 필터링된 날짜 열의 업데이트를 지원하도록 설계되지는 않았습니다. 업데이트는 업데이트가 아닌 삽입 및 삭제로 해석됩니다. 삭제가 기록 범위에서 발생하고 증분 범위가 아닌 경우에는 선택되지 않습니다. 따라서 파티션-키 충돌로 인해 데이터 새로 고침 오류가 발생할 수 있습니다.
...
- Power BI 서비스에서 증분 새로 고침 정책이 포함된 PBIX 파일을 다운로드할 수 있으면 Power BI Desktop에서 열리지 않습니다. 곧 다운로드할 수 없게 됩니다. 이는 앞으로도 지원될 수 있지만, 이러한 데이터 세트가 일반 데스크톱 PC에서 다운로드하고 열 수 없을 정도로 커질 수 있다는 점에 주의하세요.

#### <a name="refresh-ranges"></a>범위 새로 고침

다음 예에서는 만 5년 동안의 데이터와 현재 연도의 현재 날짜까지의 데이터를 저장하고 10일간의 데이터를 증분 방식으로 새로 고치는 새로 고침 정책을 정의합니다. 첫 번째 새로 고침 작업은 데이터 기록을 로드합니다. 추후 새로 고침은 증분 방식으로 이루어지며 (매일 실행되도록 예약된 경우) 다음 작업을 수행하세요.
