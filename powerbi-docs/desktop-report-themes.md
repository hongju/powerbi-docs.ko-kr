...
Power BI Desktop에서 직접 또는 테마 JSON 파일을 통해 사용자 지정하여 **서식** 창에 표시되는 거의 모든 요소를 사용자 지정하고 표준화할 수도 있습니다. 목표는 보고서 모양과 느낌을 세분화된 수준까지 완전히 제어하는 것입니다.

## <a name="how-report-themes-work"></a>보고서 테마 작동 방법
Power BI Desktop 보고서에 보고서 테마를 적용하려면 사용 가능한 기본 제공 보고서 테마 중에서 선택하거나, 사용자 지정 테마를 만들거나 가져옵니다.
...
Power BI Desktop에서 테마를 사용자 지정하는 방법에는 두 가지가 있습니다. 각 방법을 차례로 살펴보겠습니다.


## <a name="customize-report-themes-preview"></a>보고서 테마 사용자 지정(미리 보기)

**Power BI Desktop** 2019년 12월 릴리스부터 다음 두 가지 방법으로 보고서 테마를 사용자 지정할 수 있습니다.

* Power BI Desktop에서 테마 만들기 및 사용자 지정(미리 보기)
* 사용자 지정 보고서 테마 JSON 파일 만들기 및 사용자 지정

Power BI Desktop에서 직접 테마를 사용자 지정하려면 먼저 **파일 > 옵션 및 설정 > 옵션**을 선택한 다음, **미리 보기 기능** 섹션에서 다음 그림과 같이 **현재 테마 사용자 지정** 옆에 있는 확인란을 선택합니다.

![사용자 지정 테마 사용](media/desktop-report-themes/report-themes_5a.png)

미리 보기 기능을 사용하도록 설정하려면 Power BI Desktop을 다시 시작하라는 메시지가 표시될 수 있습니다.

다시 시작한 후 **홈** 리본을 선택한 다음, 리본에서 **테마 전환 > 현재 테마 사용자 지정**을 선택하여 현재 테마의 사용자 지정을 시작할 수 있습니다. 기존 테마를 사용자 지정할 수 있는 여러 가지 방법이 표시된 대화 상자가 나타납니다.

![테마 사용자 지정](media/desktop-report-themes/report-themes_5b.png)

기존 테마를 사용하고 몇 가지를 조정하려는 경우 기존 테마를 선택하고 다음 그림과 같이 대화 상자에서 **현재 테마 사용자 지정**을 선택할 수 있습니다. 

![현재 테마 사용자 지정](media/desktop-report-themes/report-themes_5c.png)

> [!NOTE]
> 이전 그림은 현재 미리 보기로 제공되는 새 리본을 사용하여 캡처되었습니다. **파일 > 옵션 및 설정 > 옵션**을 선택한 다음, **미리 보기 기능** 섹션에서 **새 리본 미리 보기**를 선택하여 새 리본 미리 보기를 사용하도록 설정할 수 있습니다.

사용자 지정할 수 있는 테마 설정은 다음 범주로 제공되며, 테마 사용자 지정 대화 상자에 반영됩니다.

* 테마 이름(사용자 지정 테마의 이름 지정) 및 다양한 색 설정(테마 색, 감정 색, 다른 색 등)
* 글꼴 패밀리, 크기, 색과 축 제목, 색, 카드 및 KPI, 탭 헤더 등의 텍스트 설정
* 배경, 테두리, 헤더, 도구 설명 등의 시각적 개체 요소
* 배경 무늬, 배경 등의 페이지 요소
* 배경색, 투명도, 글꼴 및 아이콘 색, 크기, 필터 카드 등의 필터 창 설정

변경하고 **적용 및 저장** 단추를 선택하면 테마가 저장되며, 현재 보고서에 사용하고 내보낼 수 있습니다. 

이러한 방식으로 현재 테마를 사용자 지정하면 빠르고 간편하게 테마를 사용자 지정할 수 있습니다. 그러나 다음 섹션에 설명된 것처럼 테마의 JSON 파일을 수정해야 하는 몇 가지 제한적인 테마 조정도 있습니다.

> [!TIP]
> **현재 테마 사용자 지정** 대화 상자에서 시각적 개체 요소를 사용하여 대부분의 테마 요소를 사용자 지정한 다음, JSON 파일을 내보내고 JSON 파일 자체를 직접 수정하여 미세 조정할 수 있습니다. 미세 조정된 JSON 파일의 이름을 바꾸고 가져오면 원하는 모든 조정이 완료됩니다.
