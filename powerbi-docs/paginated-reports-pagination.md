...
ms.locfileid: "68624843"
---
# <a name="pagination-in-power-bi-paginated-reports"></a>Power BI 페이지를 매긴 보고서의 페이지 매김

 페이지 매김은 보고서 내의 페이지 수와 이러한 페이지에 보고서 항목이 정렬되는 방식을 나타냅니다. Power BI 페이지를 매긴 보고서의 페이지 매김은 보고서를 보고 전송하는 데 사용하는 렌더링 확장 프로그램에 따라 달라집니다. 보고서 서버에서 보고서를 실행하면 보고서는 HTML 렌더러를 사용합니다. HTML은 특정 페이지 매김 규칙 집합을 따릅니다. 예를 들어 동일한 보고서를 PDF로 내보내면 PDF 렌더러가 사용되고 다른 규칙 집합이 적용됩니다. 따라서 보고서의 페이지를 다르게 매깁니다. 보고서를 전송하는 데 사용할 렌더러에 최적화된 사용자가 쉽게 읽을 수 있는 보고서를 성공적으로 디자인하려면 Power BI 페이지가 매겨진 보고서에서 페이지 매김을 제어하는 데 사용되는 규칙을 이해해야 합니다.  
  
 이 토픽에서는 하드 페이지 나누기 렌더러에서 보고서를 렌더링하는 방법에 대한 보고서의 레이아웃과 실제 페이지 크기의 영향에 대해 설명합니다. 실제 페이지 크기와 여백을 수정하는 속성을 설정하고, **보고서 속성** 창, **속성** 창 또는 **페이지 설정** 대화 상자를 사용하여 보고서를 열로 나눌 수 있습니다. 보고서 본문 외부의 파란색 영역을 클릭하여 **보고서 속성** 창에 액세스합니다. [홈] 탭에서 **실행**을 클릭하고 [실행] 탭에서 **페이지 설정**을 클릭하여 **페이지 설정** 대화 상자에 액세스합니다.  
...
## <a name="the-physical-page"></a>실제 페이지  
 실제 페이지 크기는 용지 크기입니다. 보고서에 지정하는 용지 크기에 따라 보고서 렌더링 방식이 제어됩니다. 하드 페이지 나누기 형식으로 렌더링된 보고서는 하드 페이지 나누기 파일 형식으로 인쇄하거나 볼 때 최적화된 읽기 환경을 제공하기 위해 실제 페이지 크기에 따라 가로 및 세로 방향으로 페이지 나누기를 삽입합니다. 소프트 페이지 나누기 형식으로 렌더링된 보고서는 웹 브라우저에서 볼 때 최적화된 읽기 환경을 제공하기 위해 실제 크기에 따라 가로로 페이지 나누기를 삽입합니다.  
  
 기본적으로 페이지 크기는 8.5 x 11인치이지만, 이 크기는 **보고서 속성** 창 또는 **페이지 설정** 대화 상자에서 변경하거나 **속성** 창에서 PageHeight 및 PageWidth 속성을 변경하여 크기를 조정할 수 있습니다. 페이지 크기는 보고서 본문의 내용에 맞게 커지거나 축소되지 않습니다. 단일 페이지에 보고서를 표시하려면 보고서 본문의 모든 내용이 실제 페이지에 맞아야 합니다. 맞지 않아 하드 페이지 나누기 형식을 사용하려는 경우 보고서에 추가 페이지가 필요합니다. 보고서 본문이 실제 페이지의 오른쪽 가장자리를 벗어나면 가로 방향으로 페이지 나누기가 삽입됩니다. 보고서 본문이 실제 페이지의 맨 아래 가장자리를 벗어나면 세로 방향으로 페이지 나누기가 삽입됩니다.  
