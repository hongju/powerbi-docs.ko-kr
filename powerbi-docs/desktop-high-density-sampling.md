...
고밀도 시각적 개체에 대해 **Power BI**는 고해상도 청크로 데이터를 지능적으로 분할한 다음 각 청크를 나타내기 위해 중요한 요소를 선택합니다. 고해상도 데이터 분할 프로세스는 결과 차트가 모든 기본 데이터 포인트를 렌더링하는 것에서 시각적으로 구분되지 않도록 특별히 조정되지만 훨씬 빠르고 더욱 상호작용적입니다.

### <a name="minimum-and-maximum-values-for-high-density-line-visuals"></a>고밀도 선 시각적 개체에 대한 최솟값 및 최대값
