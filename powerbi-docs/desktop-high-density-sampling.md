...
고밀도 시각적 개체에 대해 **Power BI**는 고해상도 청크로 데이터를 지능적으로 분할한 다음 각 청크를 나타내기 위해 중요한 요소를 선택 합니다. 고해상도 데이터 분할 프로세스는 결과 차트가 모든 기본 데이터 포인트를 렌더링하는 것에서 시각적으로 구분되지 않도록 특별히 조정되지만 훨씬 빠르고 더욱 상호작용적입니다.

### <a name="minimum-and-maximum-values-for-high-density-line-visuals"></a>최소값 및 고밀도 선 시각적 개체에 대한 최대값
...
알고리즘은 시각적 개체에 대한 가장 큰 세분성을 만들기 위해 가능한 많은 bin을 만듭니다. 각 bin 내에서 알고리즘은 최소 및 최대 데이터 값을 찾아 중요하고 의미 있는 해당 값(예: 이상값)이 시각적 개체에서 캡처되고 표시되도록 합니다. Power BI에 의한 데이터의 범주화 및 후속 평가의 결과에 따라 시각적 개체에 대한 최대 세분성을 보장하도록 시각적 개체에 대한 x 축의 최소 해상도가 결정됩니다.
...
기본적으로 새 알고리즘은 **설정**되어 있습니다. 이 설정을 변경 하려면, **서식** 창으로 이동하여, **일반** 이라는 카드의 아래쪽에서 **고밀도 샘플링** 토글 슬라이더를 볼 수 있습니다. 이 기능을 해제하려면 **해제**로 밉니다.
...
* 증가한 세분성 및 범주화 프로세스로 인해 **도구 설명**은 대표 데이터가 커서와 정렬되는 경우에만 값을 나타낼 수 있습니다. 자세한 내용은 이 문서의 *도구 설명 및 고밀도 선 샘플링*  섹션을 참조합니다.
