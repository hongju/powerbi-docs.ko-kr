...
**자신이** [Peter Myers](https://www.linkedin.com/in/peterjsmyers) (데이터 플랫폼 MVP 및 독립적인 BI 전문가 (비트 솔루션 포함)
...
다음 두 가지 방법으로 사용할 수 있습니다. 클라우드 서비스 및 **Power BI Report Server**이라는 온-프레미스 보고 솔루션 \[[1(sp1)](#endnote-01)\]
...
**용량은** Power BI 콘텐츠를 호스팅하고 전달 하는 데 사용 되는 리소스 (저장소, 프로세서 및 메모리) 집합을 나타내는 핵심 Power BI 개념입니다. 용량은 공유 또는 전용입니다. **공유 용량은** 다른 Microsoft 고객과 공유 되지만 **전용 용량은** 단일 고객에 게 완전히 커밋됩니다. 전용 용량은 [프리미엄 용량](#premium-capacities) 항목에서 도입 되었습니다.

공유 용량에서 작업은 다른 고객과 공유 하는 계산 리소스에서 실행 됩니다. 용량이 리소스를 공유 해야 하므로 최대 모델 크기 (1gb) 및 최대 일별 새로 고침 빈도 (하루 8 회)와 같이 "공평 재생"을 보장 하기 위해 제한이 적용 됩니다.

### <a name="workspaces"></a>작업 영역

Power BI 작업 영역은 용량 내에 있으며 보안, 공동 작업 및 배포 컨테이너를 나타냅니다. 각 Power BI 사용자는 **내 작업 영역**이라는 개인 작업 영역을 갖습니다. 협업 및 배포를 지원하기 위해 추가 작업 영역을 만들 수 있으며 이를 **앱 작업 영역**이라고 합니다. 기본적으로 개인 작업 영역을 포함 하는 작업 영역은 공유 용량에 생성 됩니다.
...
모든 Power BI 콘텐츠는 Power BI 콘텐츠에 대 한 컨테이너인 작업 영역 내에 저장 되 고 관리 됩니다. 각 Power BI 사용자는 자신의 개인 작업 영역을가지고 있지만 일반적인 모범 사례는 앱 작업 영역을 만드는 것입니다. 앱 작업 영역을 통해 콘텐츠 공동 소유권 및 콘텐츠 공동 작업 기능을 사용할 수 있습니다. 또한 콘텐츠를 스테이징 하 고 앱으로 다양 한 대상에 배포 하는 기능을 제공 합니다.
...
데이터 흐름는 앱 작업 영역 에서만 만들고 관리할 수 있으며 Azure Data Lake Storage Gen2의 CDM (Common Data Model)에 엔터티로 저장 됩니다. 일반적으로 최신 데이터를 저장 하기 위해 반복적으로 새로 고치도록 예약 됩니다.
...
스트리밍 데이터 집합 \[ [3](#endnote-03)\]을 제외 하 고 데이터 집합은 Analysis Services의 완성 된 모델링 기술을 활용 하는 데이터 모델을 나타냅니다.
...
Power BI Desktop-Power BI 개발을 위한 클라이언트 응용 프로그램-Analysis Services 테이블 형식 모델을 개발 하는 데 사용할 수 있습니다. 모델은 데이터 흐름에서 데이터를 가져와 개발할 수 있습니다. 그러면 다른 데이터 원본과 통합할 수 있습니다. 모델링을 구현 하는 방법에 대 한 자세한 내용은이 백서에서 다루지 않지만 Power BI Desktop를 사용 하 여 개발할 수 있는 모델에는 세 가지 유형이 있습니다. 이러한 모드는 데이터를 모델로 가져올지 아니면 데이터 원본에 유지할지를 결정 합니다. 세 가지 모드는 다음과 같습니다. 가져오기, DirectQuery 및 복합. 각 모드에 대 한 자세한 설명은 [모델 저장소 모드](#model-storage-modes) 항목에서 다룹니다.
...
Power BI 통합 문서는 Power BI 콘텐츠 형식 \[ [4](#endnote-04)\]입니다. Power BI 서비스에 업로드 된 Excel 통합 문서 이며, 데이터 집합 (모델)을 만드는 업로드 된 Excel 통합 문서와 혼동 해서는 안 됩니다. 통합 문서 콘텐츠 형식은 통합 문서에 대 한 연결을 나타냅니다 .이 통합 문서는 Power BI 서비스 업로드 하거나 OneDrive 또는 SharePoint Online의 클라우드 저장소에 유지할 수 있습니다.
...
보고서에는 다음과 같은 두 가지 유형이 있습니다. 보고서 및 페이지를 매긴 보고서를 Power BI 합니다.
...
- 일반적으로 필터 및 슬라이서 상호 작용에는 모델 쿼리가 포함 됩니다. 예를 들어 슬라이서 선택 항목을 변경 하면 기본적으로 \[ [5](#endnote-05) 페이지에서 각 시각적 개체를 다시 로드 해야 합니다.\]
...
**페이지를 매긴 보고서** 는 SQL Server Reporting Services (SSRS) 보고서 (\*.rdl 형식)의 게시 및 렌더링을 허용 합니다. 이름에서 알 수 있듯이 페이지가 매겨진 보고서는 고정 된 페이지 크기로 인쇄 해야 할 필요가 있거나 완전히 확장 되어야 하는 데이터의 변수 목록이 있는 경우 일반적으로 사용 됩니다. 예를 들어, 시각적 개체 내에서 스크롤하고 인쇄 하지 않고 다중 페이지 렌더링을 위해 디자인 된 청구서입니다.
...
- 모델은 지원 되는 단일 데이터 원본을 기반으로 해야 하므로 데이터 원본에서 데이터 통합이 이미 수행 되어야 합니다. 지원 되는 데이터 원본은 많은 인기 있는 데이터 저장소 \[ [7](#endnote-07)\]을 지 원하는 관계형 및 분석 시스템입니다.
...
**Power BI Pro** 라이선스는 개인이 앱 작업 영역 내에서 작업 영역을 만들고 공동 작업 하 고 Power BI 콘텐츠를 공유 및 배포할 수 있도록 합니다. 또한 온-프레미스 데이터 원본을 포함 하 여 데이터를 최신 상태로 유지 하도록 데이터 집합에 대 한 새로 고침을 구성할 수 있습니다. 또한 데이터에 액세스 하 고 사용 하는 방법을 감사 하 고 제어할 수 있습니다. 사용자가 전용 용량 Power BI Premium 연결 되어 있지 않으면이 라이선스는 다른 사용자의 공유 콘텐츠를 수신 하는 데 필요 합니다.
...
또 다른 방법은 단일 SKU 제품군을 포함 하는 Azure Power BI Embedded 구독을 구매 하는 것입니다. 포함 및 용량 테스트 목적 으로만 사용 되 **는** Sku (A1-A6)입니다.

모든 sku는 v 코어를 제공 하 여 \[용량 [9](#endnote-09)\]를 만들지만, EM sku는 작은 규모의 포함을 위해 제한 됩니다. 이 백서에 집중 하는 것은 P Sku에 대 한 것 이지만 설명 된 많은 내용이 Sku에도 관련이 있습니다.
...
구독 및 라이선스 항목에 설명 된 대로 두 가지 Power BI Premium SKU 제품군이 있습니다. EM 및 P 모든 Power BI Premium Sku는 프로세서, 메모리 및 저장소로 구성 된 리소스의 집합 양을 나타내는 용량 노드로 사용할 수 있습니다. 리소스 외에도 각 SKU는 초당 DirectQuery (DQ) 및 라이브 연결 (LC) 연결 수 및 병렬 모델 새로 고침 수에 대 한 작업 제한을 갖습니다.
...
가져오기 모델을 쿼리하거나 새로 고칠 수 있도록 메모리에 완전히 로드 해야 합니다. Power BI 서비스는 사용 가능한 메모리의 최대 사용을 보장 하기 위해 정교한 알고리즘을 사용 하 여 메모리 사용을 관리 하 고 용량을 과도 하 게 커밋하는 데 사용할 수 있습니다. 용량이 많은 가져오기 모델을 저장할 수 있지만 (프리미엄 용량 당 최대 100 TB) 결합 된 디스크 저장소가 지원 되는 메모리를 초과 하면 (그리고 쿼리 및 새로 고침을 위해 추가 메모리가 필요 함)에는 모두 메모리에 로드할 수 없습니다. 시간이 동일 합니다.

따라서 가져오기 모델은 사용에 따라 메모리에서 로드 되 고 제거 됩니다. 가져오기 모델은 쿼리 (대화형 작업) 되 고 아직 메모리에 있지 않은 경우 또는 새로 고쳐질 때 (백그라운드 작업) 로드 됩니다.

메모리에서 모델을 제거 하는 것을 제거  라고 하며,이 작업은 모델의 크기에 따라 Power BI 빠르게 수행할 수 있는 작업입니다. 용량에 메모리 압력이 없을 경우 모델은 메모리에 로드되어 그대로 남아 있습니다. \[[10 그러나 모델](#endnote-10) 을 로드 하는 데 사용할 수 있는 메모리가 부족 한 경우 Power BI 서비스 먼저 메모리를 확보 해야 합니다.\] 지난 3 분 \[ [11](#endnote-11)\]동안 사용 되지 않은 모델을 검색 하 고 제거 하 여 비활성화 된 모델을 검색 하 여 메모리를 확보 합니다. 제거할 비활성 모델이 없는 경우 Power BI 서비스는 백그라운드 작업을 위해 로드된 모델을 제거하려고 시도합니다. 여기에는 AI 워크 로드와 같은 백그라운드 워크 로드의 제거도 포함 될 수 있습니다. 실패 \[ [한](#endnote-11)시도\]의 30 초 후 30 초 후에는 대화형 작업을 수행할 수 없습니다. 이 경우 보고서 사용자는 잠시 후 다시 시도 하는 제안에 대 한 실패를 알려 줍니다.
...
- 용량 관리자
- 용량 크기
- 데이터 상주 \[에 대 한 지역 [12](#endnote-12)\]

용량 관리자를 하나 이상 할당 해야 합니다. 용량 관리자로 할당 된 사용자는 다음을 수행할 수 있습니다.

- 용량에 작업 영역 할당
- 사용자 권한 관리-할당 권한이 있는 용량 관리자 또는 사용자를 추가 하 여 작업 영역을 용량에 할당할 수 있도록 합니다.
- 작업을 관리 하 여 페이지를 매긴 보고서 및 데이터 흐름 작업에 대 한 최대 메모리 사용량 구성
- 시스템 오버 로드 \[ [13](#endnote-13) 의 경우 모든 작업을 다시 설정 하려면 용량을 다시 시작 합니다.\]

작업 영역 권한이 명시적으로 할당 되지 않은 경우 용량 관리자는 작업 영역 콘텐츠에 액세스할 수 없으며, 사용 메트릭, 감사 로그 또는 테 넌 트 설정과 같은 모든 Power BI 관리 영역 (명시적으로 할당 되지 않은 경우)에 액세스할 수 없습니다. 중요 한 것은 용량 관리자에 게 새 용량을 만들거나 기존 용량을 확장할 수 있는 권한이 없는 것입니다. 또한 할당 된 용량을 확인 하 고 관리할 수 있도록 보장 하기 위해 용량 기준에 따라 할당 됩니다.

풀에서 사용 가능한 v 코어 수에 의해 제한 되는 사용 가능한 SKU 옵션 목록에서 용량 크기를 선택 해야 합니다. 하나 이상의 구매한 Sku에서 원본으로 사용할 수 있는 풀에서 여러 용량을 만들 수 있습니다. 예를 들어 P3 SKU (32 v 코어)를 사용 하 여 3 개의 용량을 만들 수 있습니다. 하나는 P2 (16 개 코어)이 고, 두 개의 P1 (2 x 8 v 코어)입니다. 더 작은 크기의 용량을 만들어 성능 및 확장성을 향상 시킬 수 있습니다 .이 항목은 [프리미엄 용량 최적화](#optimizing-premium-capacities) 섹션에 설명 되어 있습니다. 다음 이미지는 포함 하는 각각의 앱 작업 영역과 공유 용량의 여러 작업 영역을 포함 하는 5 개의 프리미엄 용량 (3 x P1 및 2 x P3)으로 구성 된 가상의 Contoso 조직에 대 한 설정 예를 보여 줍니다.

![가상의 Contoso 조직에 대 한 예제 설정](media/whitepaper-premium-deployment/contoso-organization-example.png)

프리미엄 용량은 Power BI 테 넌 트의 홈 지역이 아닌 지역에 할당 될 수 있으며, 정의 된 지리적 Power BI 지역 내에서 콘텐츠가 상주 하는 데이터 센터에 대 한 관리 제어를 제공 합니다. \[[10](#endnote-12)\]

Power BI 서비스 관리자와 Office 365 전역 관리자는 프리미엄 용량을 수정할 수 있습니다. 특히 다음을 수행할 수 있습니다.
...
할당 권한은 특정 프리미엄 용량에 작업 영역을 할당 하는 데 필요 합니다. 이 권한은 전체 조직, 특정 사용자 또는 그룹에 게 부여할 수 있습니다.

기본적으로 프리미엄 용량은 실행 중인 Power BI 쿼리와 관련 된 워크 로드를 지원 합니다. 또한 세 가지 추가 작업을 지원 합니다. **페이지를 매긴 보고서**, **데이터 흐름**및 **AI** 각 워크 로드에는 작업에서 사용할 수 있는 최대 메모리 (총 사용 가능한 메모리의 백분율)를 구성 해야 합니다. 최대 메모리 할당 증가가 호스트할 수 있는 활성 모델 수와 새로 고침 처리량에 영향을 줄 수 있다는 것을 이해 하는 것이 중요 합니다.

메모리는 데이터 흐름에는 동적으로 할당되지만 페이지를 매긴 보고서에는 정적으로 할당됩니다. 최대 메모리를 정적으로 할당 하는 이유는 페이지가 매겨진 보고서가 용량이 포함 된 보안 공간 내에서 실행 된다는 것입니다. 모델을 로드 하는 데 사용할 수 있는 메모리를 줄임으로써 페이지가 매겨진 보고서 메모리를 설정할 때는 주의 해야 합니다.

|                     | EM3                      | P1                       | P2                      | P3                       |
|---------------------|--------------------------|--------------------------|-------------------------|--------------------------|
| 페이지가 매겨진 보고서 | N/A | 기본값 20%, 최솟값 10% | 기본값 20%, 최솟값 5% | 기본값 20%, 최솟값 2.5% |
| 데이터 흐름 | 기본값 20%, 최솟값 8%  | 기본값 20%, 최솟값 4%  | 기본값 20%, 최솟값 2% | 기본값 20%, 최솟값 1%  |
| AI | 해당 사항 없음 | 20% 기본값; 20% 최소값  | 기본값 20%, 최솟값 10% | 기본값 20%, 최솟값 5%  |
| | | | | |
...
작업 영역은 **작업** 영역 창에서 **Power BI 관리** **포털** 또는 앱 작업 영역에 대 한 프리미엄 용량에 할당할 수 있습니다.

용량 관리자는 물론 Office 365 전역 관리자 또는 Power BI 서비스 관리자는 **Power BI 관리**  **포털**에서 작업 영역을 대량으로 할당할 수 있습니다. 대량 할당은 다음에 적용 될 수 있습니다.

- **사용자 별 작업 영역** : 개인 작업 영역을 포함 하 여 해당 사용자가 소유 하는 모든 작업 영역이 프리미엄 용량에 할당 됩니다. 이 작업 영역에는 이미 다른 프리미엄 용량에 할당 된 경우 작업 영역을 다시 할당 하는 작업이 포함 됩니다. 또한 사용자에 게 작업 영역 할당 권한도 할당 됩니다.

- **특정 작업 영역**
- **전체 조직의 작업 영역** : 개인 작업 영역을 비롯 한 모든 작업 영역이 프리미엄 용량에 할당 됩니다. 또한 모든 현재 및 미래의 사용자에 게 작업 영역 할당 권한이 할당 됩니다. \[[14](#endnote-14)\]

사용자가 작업 영역 관리자이 고 할당 권한이 있는 **작업** 영역 창을 사용 하 여 작업 영역을 프리미엄 용량에 추가할 수 있습니다.

![작업 영역 창을 사용 하 여 프리미엄 용량에 작업 영역 할당](media/whitepaper-premium-deployment/assign-workspace-capacity.png)

작업 영역 관리자는 할당 권한이 없어도 용량 (공유 용량)에서 작업 영역을 제거할 수 있습니다. 전용 용량에서 작업 영역을 제거 하면 작업 영역을 공유 용량에 효과적으로 재배치 합니다. 프리미엄 용량에서 작업 영역을 제거 하면 부정적인 결과가 발생할 수 있습니다. 예를 들어, 사용이 허가 된 사용자를 Power BI 수 없는 공유 콘텐츠 또는 지원 되는 허용량을 초과 하는 경우 예약 된 새로 고침이 일시 중단 될 수 있습니다. 공유 용량을 기준으로 합니다.

Power BI 서비스에서 프리미엄 용량에 할당 된 작업 영역은 작업 영역 이름을 원으로 하는 다이아몬드 아이콘으로 쉽게 식별할 수 있습니다.

![프리미엄 용량에 할당 된 작업 영역 식별](media/whitepaper-premium-deployment/premium-diamond-icon.png)

### <a name="monitoring-capacities"></a>용량 모니터링

프리미엄 용량 모니터링은 관리자에 게 용량을 수행 하는 방법을 이해 하는 데 사용 됩니다. [용량 Power BI Premium 용량 메트릭 앱](service-admin-premium-monitor-capacity.md) 또는 [Power BI 관리 포털](service-admin-premium-monitor-portal.md)을 사용 하 여 모니터링할 수 있습니다.

#### <a name="interpreting-metrics"></a>메트릭 해석

리소스 사용 및 작업 활동에 대 한 기준선 이해를 설정 하려면 메트릭을 모니터링 해야 합니다. 용량이 느려지는 경우 모니터링할 메트릭과 수행할 수 있는 결론을 이해 하는 것이 중요 합니다.

이상적으로는 쿼리를 완료 하 여 보고서 사용자에 게 응답성이 뛰어난 환경을 제공 하 고 더 높은 쿼리 처리량을 사용 하는 것이 좋습니다. 일반적으로 백그라운드 프로세스 (새로 고침 포함)를 완료 하는 데 시간이 더 오래 걸립니다.

일반적으로 느리게 표시 되는 보고서는 과열 된 용량을 나타낼 수 있습니다. 보고서를 로드 하지 못하면이는 과도 하 게 열 된 용량을 나타냅니다. 어느 경우 든 근본 원인은 다음을 비롯 한 다양 한 요인으로 인해 발생할 수 있습니다.

- **실패 한 쿼리** 는 메모리 압력을 나타내며 메모리에 모델을 로드할 수 없습니다. Power BI 서비스는 실패 하기 전에 30 초 동안 모델을 로드 하려고 시도 합니다.

- **과도 한 쿼리 대기 시간은** 다음과 같은 원인으로 인해 발생할 수 있습니다.
...
- **긴 쿼리 기간은** 모델 디자인이 최적화 되지 않았음을 나타낼 수 있습니다. 특히 하나의 용량에서 여러 데이터 집합이 활성화 되 고 하나의 데이터 집합만 긴 쿼리 기간을 생성 하는 경우입니다. 이는 용량에 대 한 충분 한 데이터를 갖고 있으며, 질문에 대 한 데이터 집합이 최적화 되어 있거나 속도가 느립니다. 장기 실행 쿼리는 다른 프로세스에 필요한 리소스에 대 한 액세스를 차단할 수 있으므로 문제가 될 수 있습니다.
...
최상의 사용률 및 성능을 얻기 웹사이트 때 보드에서 일반적인 권장 사항으로 수행할 수 있는 몇 가지 모범 사례가 있습니다. 이러한 개체는 다음과 같습니다.

- 개인 작업 영역 대신 앱 작업 영역 사용
- 중요 비즈니스용 SSBI (셀프 서비스 BI)를 다른 용량으로 분리

  ![비즈니스에 중요 한 서비스와 셀프 서비스 BI를 서로 다른 용량으로 분리](media/whitepaper-premium-deployment/separate-capacities.png)
...
#### <a name="why-is-the-capacity-slow-and-what-can-i-do"></a>용량이 느려지는 이유와 수행할 수 있는 작업은 무엇 인가요?

프리미엄 용량에 영향을 주는 많은 이유가 있습니다. 이 질문에는 저속의 의미를 이해 하기 위해 추가 정보가 필요 합니다. 보고서를 로드 하는 속도가 느림? 또는 로드에 실패 하나요? 사용자가 보고서와 상호 작용할 때 보고서 시각적 개체가 로드 또는 업데이트 되는 속도가 느리다? 새로 고침이 예상 보다 더 오래 걸리거나 이전에 경험이 있나요?

이유를 이해 하 고 나면 조사를 시작할 수 있습니다. 다음 6 가지 질문에 대 한 응답은 보다 구체적인 문제를 해결 하는 데 도움이 됩니다.

#### <a name="what-content-is-using-up-my-capacity"></a>용량을 사용 하는 콘텐츠는 무엇 인가요?

용량을 기준으로 필터링 하는 **Power BI Premium 용량 메트릭** 앱을 사용 하 고 작업 영역 콘텐츠에 대 한 성능 메트릭을 검토할 수 있습니다. 프리미엄 용량 내에 저장 된 모든 콘텐츠에 대해 지난 7 일간의 시간별 성능 메트릭 및 리소스 사용량을 검토할 수 있습니다. 이는 프리미엄 용량 성능에 대 한 일반적인 문제를 해결할 때 첫 번째로 수행 해야 하는 단계입니다.
...
또한 Power BI Premium 용량 메트릭 앱에서 활성 메모리는 지난 3 분 동안 사용 중 이므로 제거할 수 없는 보고서에 할당 된 총 메모리 양을 보여 줍니다. 새로 고침 대기 시간에 높은 스파이크가 있으면 큰 및/또는 활성 데이터 집합의 상관 관계를 지정할 수 있습니다.

"평균 기간별 상위 5 개" 차트는 상위 5 개 데이터 집합, 페이지를 매긴 보고서, 데이터 흐름 및 AI 호출을 소비 하는 용량 리소스를 강조 표시 합니다. 상위 5 개 목록의 콘텐츠는 조사 및 가능한 최적화 후보입니다.

#### <a name="why-are-reports-slow"></a>보고서가 느리게 표시 되는 이유는 무엇 인가요?

다음 표에서는 가능한 문제와 이러한 문제를 식별 하 고 처리 하는 방법을 보여 줍니다.

##### <a name="insufficient-capacity-resources"></a>용량 리소스가 부족 합니다.

| 가능한 설명 | 확인 방법 | 해결 방법 |
| --- | --- | --- |
| 높은 총 활성 메모리 (모델을 지난 3 분 동안 사용 중이기 때문에 제거할 수 없음)<br><br> 쿼리 대기 시간에 대 한 여러 개의 높은 급증<br><br> 새로 고침 대기 시간에 대 한 여러 개의 높은 급증 | 메모리 메트릭 \[ [18](#endnote-18)\]및 제거 횟수 \[ [19](#endnote-19) 모니터링\] | 모델 크기를 줄이거나 DirectQuery 모드로 전환 합니다 .이 섹션의 [모델 최적화](#optimizing-models) 항목을 참조 하세요.<br><br> 용량 강화<br><br> 다른 용량에 콘텐츠 할당 |

##### <a name="inefficient-report-designs"></a>비효율적인 보고서 디자인

| 가능한 설명 | 확인 방법 | 해결 방법 |
| --- | --- | --- |
| 보고서 페이지에 많은 시각적 개체가 포함 되어 있습니다. 대화형 필터링은 시각적 개체 당 하나 이상의 쿼리를 트리거할 수 있습니다.<br><br> 시각적 개체가 필요한 것 보다 많은 데이터를 검색 합니다. | 보고서 디자인 검토<br><br> 인터뷰 보고서 사용자가 보고서와 상호 작용 하는 방식을 이해 합니다.<br><br> 데이터 집합 쿼리 메트릭 \[ [20](#endnote-20)\] | 페이지당 시각적 개체가 많지 않은 보고서 다시 디자인 |
...
| 가능한 설명 | 확인 방법 | 해결 방법 |
| --- | --- | --- |
| 높은 쿼리 대기 시간<br><br> CPU 포화<br><br> DQ/LC 연결 제한을 초과 했습니다. | CPU \[사용률 [21](#endnote-21)\], 쿼리 대기 시간, DQ/LC 사용률 \[ [22](#endnote-22) \] 메트릭 + 쿼리 기간 – 변동에서 동시성 문제를 나타낼 수 있는 경우 | 용량을 확장 하거나 다른 용량에 콘텐츠를 할당 합니다.<br><br> 페이지당 시각적 개체가 많지 않은 보고서 다시 디자인 |

#### <a name="why-are-reports-not-loading"></a>보고서가 로드 되지 않는 이유는 무엇 인가요?

보고서를 로드 하지 못하는 경우에는 최악의 시나리오 이며, 용량에 메모리가 부족 하 여 과도 하 게 열 수 있습니다. 이 문제는 로드 된 모든 모델이 적극적으로 쿼리 되 고 있으므로 제거할 수 없고 새로 고침 작업이 일시 중지 되거나 지연 된 경우에 발생할 수 있습니다. Power BI 서비스는 30 초 동안 데이터 집합을 로드 하려고 시도 하며, 잠시 후에 다시 시도 하는 제안을 통해 사용자에 게 오류를 정상적으로 알립니다.

현재 보고서 로드 실패에 대해 모니터링할 메트릭이 없습니다. 시스템 메모리, 특히 가장 높은 사용률 및 최고 사용률을 모니터링 하 여이 문제에 대 한 가능성을 파악할 수 있습니다. 높은 데이터 집합 제거 및 긴 데이터 집합 새로 고침 평균 대기 시간으로 인해이 문제가 발생 하는 것이 좋습니다.

매우 가끔씩만 발생 하는 경우에는 우선 순위 문제로 간주 되지 않을 수 있습니다. 보고서 사용자는 서비스가 사용 중 이며 잠시 후 다시 시도해 야 한다는 것을 알 수 있습니다. 이 작업이 너무 자주 발생 하는 경우 프리미엄 용량을 확장 하거나 다른 용량에 콘텐츠를 할당 하 여 문제를 해결할 수 있습니다.

용량 관리자 (및 Power BI 서비스 관리자)는 **쿼리 오류** 메트릭을 모니터링 하 여이 상황이 발생 하는 시기를 결정할 수 있습니다. 시스템 오버 로드의 경우 모든 작업을 다시 설정 하 여 용량을 다시 시작할 수도 있습니다.

#### <a name="why-are-refreshes-not-starting-on-schedule"></a>새로 고침이 일정에 따라 시작 되지 않는 이유는 무엇 인가요?

예약 된 새로 고침 시작 시간이 보장 되지 않습니다. Power BI 서비스은 백그라운드 작업을 통해 항상 대화형 작업의 우선 순위를 지정 합니다. 새로 고침은 다음 두 조건이 충족 될 때 발생할 수 있는 백그라운드 작업입니다.

- 메모리가 충분 합니다.
- 프리미엄 용량에 대해 지원 되는 동시 새로 고침 수를 초과 하지 않습니다.

조건이 충족 되지 않으면 조건이 이익이 될 때까지 새로 고침이 대기 합니다.

전체 새로 고침을 수행 하려면 적어도 두 개의 현재 데이터 집합 메모리 크기가 필요 하다는 점을 기억 하세요. 충분 한 메모리를 사용할 수 없는 경우 모델 제거에서 메모리를 확보할 때까지 새로 고침을 시작할 수 없습니다 .이는 하나 이상의 데이터 집합이 비활성 상태가 되 고 제거 될 때까지 지연 됨을 의미 합니다.

지원 되는 최대 동시 새로 고침 수는 백 엔드 v 코어의 1.5 배가 되도록 설정 되어 있습니다.

예약 된 새로 고침이 시작 될 때까지 예약 된 새로 고침이 시작 되지 않으면 실패 합니다. UI에서 수동으로 트리거되는 주문형 새로 고침은 실패 하기 전까지 최대 3 회 실행을 시도 합니다.

용량 관리자 및 Power BI 서비스 관리자는 **평균 새로 고침 대기 시간 (분)** 메트릭을 모니터링 하 여 예약 된 시간과 작업 시작 사이의 평균 지연 시간을 확인할 수 있습니다.

일반적으로 관리 우선 순위가 아니라 시간에 데이터 새로 고침에 영향을 주기 위해 충분 한 메모리를 사용할 수 있는지 확인 합니다. 여기에는 알려진 충분 한 리소스로 데이터 집합을 용량으로 격리 하는 작업이 포함 될 수 있습니다. 또한 관리자가 데이터 집합 소유자와 조정 하 여 충돌을 최소화 하기 위해 예약 된 데이터 새로 고침 시간을 줄이거나 줄일 수 있습니다. 관리자가 새로 고침 큐를 보거나 데이터 집합 일정을 검색 하는 것은 불가능 합니다.

#### <a name="why-are-refreshes-slow"></a>새로 고침이 느려지는 이유는 무엇입니까?

새로 고침은 느리거나 느리게 인식 될 수 있습니다 (이전의 일반적인 질문 주소).

새로 고침이 실제로 느려지는 경우 다음과 같은 몇 가지 이유로 인해 발생할 수 있습니다.
...
용량 관리자 및 Power BI 서비스 관리자는 **평균 새로 고침 기간 (분)** 메트릭을 모니터링 하 여 시간에 따른 비교에 대 한 벤치 마크 및 평균 지연 **시간 (분)** 메트릭을 확인할 수 있습니다. 예약 된 시간과 작업 시작 사이의 평균 지연 사이입니다.

증분 새로 고침은 특히 큰 모델 테이블의 경우 데이터 새로 고침 기간을 크게 줄일 수 있습니다. 증분 새로 고침과 관련 된 네 가지 이점은 다음과 같습니다.

- **새로 고침은 더 빠릅니다** . 여러 파티션을 새로 고칠 때 테이블의 하위 집합만 로드 하 고, CPU 및 메모리 사용량을 줄이고, 병렬 처리를 더 높일 수 있습니다.
- **새로 고침은 필요한 경우에만 발생** 합니다. 증분 새로 고침 정책은 데이터가 변경 된 경우에만 로드 되도록 구성할 수 있습니다.
- **새로 고침은 더 안정적입니다** . 휘발성 데이터 원본 시스템에 대 한 실행 시간이 짧으면 연결이 끊어질 가능성이 낮습니다.
- **모델은 트리밍** 상태를 유지 합니다. 슬라이딩 기간 외의 기록을 자동으로 제거 하도록 증분 새로 고침 정책을 구성할 수 있습니다.

자세한 내용은 [Power BI Premium 문서의 증분 새로 고침](service-premium-incremental-refresh.md) 을 참조 하세요.

#### <a name="why-are-data-refreshes-not-completing"></a>데이터 새로 고침이 완료 되지 않는 이유는 무엇 인가요?

데이터 새로 고침이 시작 됩니다 완료 되지 않는 경우 다음과 같은 몇 가지 이유로 인해 발생할 수 있습니다.

- 프리미엄 용량에 모델이 하나만 있는 경우 (예: 모델 크기가 매우 큼)에도 메모리가 부족 합니다.
- 데이터 원본 시스템 연결 끊기, 잘못 된 권한 또는 게이트웨이 오류를 비롯 한 용량이 아닌 이유

용량 관리자 (및 Power BI 서비스 관리자)는 **메모리 부족 메트릭으로 인 한 새로 고침 오류** 를 모니터링할 수 있습니다.
...
최적의 모델 디자인은 효율적이 고 확장 가능한 솔루션을 제공 하는 데 매우 중요 합니다. 그러나 전체 토론을 제공 하기 위해이 백서에서는 다루지 않습니다. 대신이 섹션에서는 모델을 최적화할 때 고려해 야 할 주요 영역을 제공 합니다.
...
가져오기 모델에 대해 최적화 가능성을 고려 합니다.

![가져오기 모델에 대 한 최적화 가능성](media/whitepaper-premium-deployment/import-model-optimizations.png)
...
- 쿼리 디자인 파워 쿼리 복잡 한 변환과 특히 서로 다른 데이터 소스를 병합 하는 데이터 웨어하우스 (추출-변환-로드 단계에서이를 수행 하는 데이터 웨어하우스)를 최소화 하거나 제거할 수 있습니다. 또한 적절 한 데이터 원본 개인 정보 수준이 설정 되었는지 확인 하면 전체 결과를 로드 하 여 쿼리 간에 결합 된 결과를 생성 하는 Power BI 필요 하지 않을 수 있습니다.
- 모델 구조는 로드할 데이터를 결정 하며 모델 크기에 직접적인 영향을 줍니다. 열을 제거 하거나, 행을 제거 하거나 (특히 기록 데이터), 요약 된 데이터를 로드 하 여 (상세 데이터를 로드 하는 비용) 불필요 한 데이터를 로드 하지 않도록 디자인할 수 있습니다. 매우 효율적으로 저장 하거나 압축 하지 않는 높은 카디널리티 열 (특히 텍스트 열)을 제거 하 여 크기를 대폭 줄일 수 있습니다.
- 양방향 필터링을 허용 해야 하는 이유가 없을 경우 단일 방향 관계를 구성 하 여 모델 쿼리 성능을 향상 시킬 수 있습니다. 양방향 필터링 대신 CROSSFILTER 함수를 사용 하는 것도 고려 합니다.
- 집계 테이블은 미리 요약 된 데이터를 로드 하 여 쿼리 응답을 빠르게 수행할 수 있지만이로 인해 모델의 크기가 증가 하 고 새로 고침 시간이 길어질 수 있습니다. 일반적으로 매우 큰 모델 또는 복합 모델 디자인을 위해 집계 테이블을 예약 해야 합니다.
- 계산 된 테이블 및 열은 모델 크기를 늘리고 더 긴 새로 고침 시간이 발생 합니다. 일반적으로 데이터 원본에서 데이터를 구체화 하거나 계산 하는 경우 더 작은 저장소 크기와 빠른 새로 고침 시간을 얻을 수 있습니다. 가능 하지 않은 경우 파워 쿼리 사용자 지정 열을 사용 하면 향상 된 저장소 압축을 제공할 수 있습니다.
- 비용이 많이 드는 수식을 방지 하기 위해 논리를 다시 작성 하는 것 처럼 측정값과 RLS 규칙에 대 한 DAX 식을 튜닝할 수 있습니다.
- 증분 새로 고침은 새로 고침 시간을 크게 줄이고 메모리와 CPU를 절약할 수 있습니다. 또한 모델 크기 트리밍을 유지 하는 기록 데이터를 제거 하도록 증분 새로 고침을 구성할 수 있습니다.
- 모델은 서로 다른 쿼리 패턴 및 충돌 하는 경우 두 개의 모델로 다시 디자인 될 수 있습니다. 예를 들어 일부 보고서는 모든 기록에 대 한 높은 수준의 집계를 제공 하 고 24 시간의 대기 시간을 허용할 수 있습니다. 다른 보고서는 오늘날의 데이터와 관련 되어 있으며 개별 트랜잭션에 대 한 세분화 된 액세스가 필요 합니다. 모든 보고서를 충족 하는 단일 모델을 설계 하는 대신 각 요구 사항에 대해 최적화 된 두 개의 모델을 만듭니다.

DirectQuery 모델에 대 한 최적화 가능성을 고려 합니다. 모델은 기본 데이터 원본에 대 한 쿼리 요청을 실행 하므로 응답성이 뛰어난 모델 쿼리를 제공 하는 데에는 데이터 원본 최적화가 중요 합니다.

 ![DirectQuery 모델에 대 한 최적화 가능성](media/whitepaper-premium-deployment/direct-query-model-optimizations.png)
...
- 양방향 필터링을 허용 해야 하는 이유가 없을 경우 단일 방향 관계를 구성 하 여 모델 쿼리 성능을 향상 시킬 수 있습니다. 또한 참조 무결성이 적용 되는 것으로 가정 하 고 (이 경우) 외부 조인 대신 더 효율적인 내부 조인을 사용 하 여 데이터 원본 쿼리를 생성 하도록 모델 관계를 구성 해야 합니다.
- 파워 쿼리 쿼리 사용자 지정 열 또는 모델 계산 열 생성 안 됨-가능 하면 데이터 원본에서 이러한 열을 구체화 합니다.
- 비용이 많이 드는 수식을 방지 하기 위해 논리를 다시 작성 하는 것 처럼 측정값과 RLS 규칙에 대 한 DAX 식을 튜닝할 수 있습니다.

복합 모델에 대 한 최적화 가능성을 고려 합니다. 복합 모델에서는 가져오기 및 DirectQuery 테이블을 함께 사용할 수 있습니다.
...
- 일반적으로는 차원 유형 테이블 (비즈니스 엔터티를 나타냄)을 이중 저장소 모드와 팩트 유형 테이블 (종종 작업 팩트를 나타내는 대량 테이블)을 DirectQuery 저장소 모드로 구성 하 여 균형 있는 디자인을 구현 합니다. 이중 저장소 모드는 가져오기 및 DirectQuery 저장소 모드를 모두 의미 하며,이를 통해 Power BI 서비스는 통과를 위한 기본 쿼리를 생성할 때 사용할 가장 효율적인 저장소 모드를 결정할 수 있습니다.
- 게이트웨이는 네트워크 대역폭이 충분 하 고 데이터 원본에 근접 하 게 가까운 전용 컴퓨터에 충분 한 리소스를 보유 하 고 있는지 확인 합니다.
- 가져오기 저장소 모드로 구성 된 집계 테이블은 DirectQuery 저장소 모드 팩트 유형 테이블을 요약 하는 데 사용 될 때 뛰어난 쿼리 성능 향상을 제공할 수 있습니다. 이 경우 집계 테이블은 모델의 크기를 늘리고 새로 고침 시간을 증가 시킵니다. 일반적으로이는 더 빠른 쿼리에 사용할 수 있는 절충입니다.

#### <a name="optimizing-externally-hosted-models"></a>외부적으로 호스트 된 모델 최적화

[Power BI 호스 티 드 모델 최적화](#optimizing-power-bi-hosted-models) 항목에서 설명 하는 많은 최적화 가능성은 Azure Analysis Services 및 SQL Server Analysis Services를 사용 하 여 개발 된 모델에도 적용 됩니다. 지우기 예외는 복합 모델 및 집계 테이블을 포함 하 여 현재 지원 되지 않는 특정 기능입니다.

외부에서 호스트 되는 데이터 집합에 대 한 추가 고려 사항은 Power BI 서비스와 관련 된 데이터베이스 호스팅입니다. Azure Analysis Services의 경우,이는 Power BI 테 넌 트 (홈 지역)와 동일한 지역에 Azure 리소스를 만드는 것을 의미 합니다. IaaS의 경우,이는 동일한 지역에서 VM을 호스트 하는 것을 의미 하 고 온-프레미스의 경우 효율적인 게이트웨이 설치를 보장 하는 것을 의미 합니다. SQL Server Analysis Services

그 외에도 데이터베이스 및 SQL Server Analysis Services 테이블 형식 데이터베이스를 Azure Analysis Services 하 여 해당 모델을 메모리에 완전히 로드 하 고 쿼리를 지원 하기 위해 항상 해당 모델을 유지 해야 한다는 점에 유의 해야 할 수 있습니다. Power BI 서비스와 마찬가지로 새로 고치는 동안 모델을 온라인 상태로 유지 해야 하는 경우 새로 고침을 위한 충분 한 메모리가 필요 합니다. Power BI 서비스와 달리 모델은 사용량에 따라 자동으로 메모리에 자동으로 에이징됩니다. 따라서 Power BI Premium을 사용 하면 보다 효율적인 방법으로 메모리 사용을 줄여 모델 쿼리를 최대화할 수 있습니다.

### <a name="capacity-planning"></a>용량 계획

프리미엄 용량의 크기는 사용 가능한 메모리와 프로세서 리소스 및 용량에 적용 되는 제한을 결정 합니다. 프리미엄 용량 수를 고려 하는 것은 여러 프리미엄 용량을 만들 때 작업을 서로 격리 하는 데 도움이 될 수도 있습니다. 저장소는 용량 노드당 100 TB 이며 모든 워크 로드에 대해 충분 한 것일 수 있습니다.

프리미엄 용량 크기와 수를 결정 하는 것이 어려울 수 있습니다. 특히 초기 용량을 만드는 것이 좋습니다. 용량 크기를 조정 하는 첫 번째 단계는 예상 되는 일일 사용량을 나타내는 평균 작업량을 이해 하는 것입니다. 모든 작업이 동일 하지는 않는다는 것을 이해 하는 것이 중요 합니다. 예를 들어, 100 스펙트럼 한쪽 끝에서 단일 시각적 개체를 포함 하는 단일 보고서 페이지에 액세스 하는 동시 사용자를 쉽게 달성할 수 있습니다. 아직 스펙트럼의 다른 쪽 끝에 있는 경우-100 여러 보고서에 100 액세스 하는 동시 사용자가 각각 보고서 페이지의 100 시각적 개체를 사용 하 여 용량 리소스에 대 한 요구 사항이 서로 다를 수 있습니다.

따라서 용량 관리자는 사용자 환경, 콘텐츠 및 예상 사용량과 관련 하 여 여러 가지 요인을 고려해 야 합니다. 재정의 목표는 일관성 있는 쿼리 시간, 허용 가능한 대기 시간 및 제거 비율을 제공 하면서 용량 사용률을 최대화 하는 것입니다. 고려해 야 할 요인은 다음과 같습니다.

- **모델 크기 및 데이터 특성** : 쿼리 또는 새로 고침을 허용 하려면 가져오기 모델을 메모리로 완전히 로드 해야 합니다. LC/DQ 데이터 집합은 복잡 한 측정값 또는 RLS 규칙을 평가 하는 데 상당한 프로세서 시간 및 많은 메모리가 필요할 수 있습니다. 메모리 및 프로세서 크기와 LC/DQ 쿼리 처리량은 용량 크기로 제한 됩니다.
- **동시 활성 모델** : 서로 다른 가져오기 모델을 동시에 쿼리 하는 경우 메모리에 유지 되 면 최상의 응답성과 성능을 제공 합니다. 자주 쿼리 되는 모든 모델을 호스트 하는 데 충분 한 메모리가 있어야 하며, 새로 고칠 수 있도록 추가 메모리가 있어야 합니다.
- **모델 새로 고침 가져오기** : 파워 쿼리 쿼리 및 계산 된 테이블/열 논리의 새로 고침 유형 (전체 또는 증분), 기간 및 복잡도는 메모리 및 특히 프로세서 사용에 영향을 줄 수 있습니다. 동시 새로 고침은 용량 크기 (1.5 x 백 엔드 v 코어, 반올림)에 의해 제한 됩니다.
- **동시 쿼리** : 프로세서 또는 LC/DQ 연결이 용량 제한을 초과 하는 경우 많은 동시 쿼리를 실행 하면 보고서가 응답 하지 않을 수 있습니다. 이는 특히 많은 시각적 개체를 포함 하는 보고서 페이지의 경우입니다.
- **데이터 흐름, 페이지를 매긴 보고서 및 AI 함수** : 용량 메모리의 구성 가능한 최대 비율을 요구 하는 데이터 흐름, 페이지를 매긴 보고서 및 AI 기능을 지원 하도록 용량을 구성할 수 있습니다. 메모리가 데이터 흐름에 동적으로 할당 되지만 페이지가 매겨진 보고서와 AI 작업에 정적으로 할당 됩니다.

용량 관리자는 이러한 요소 외에도 여러 용량을 만들 수 있습니다. 여러 용량을 사용 하 여 작업을 격리할 수 있으며, 우선 순위 워크 로드에서 리소스가 보장 되도록 구성할 수 있습니다. 예를 들어, SSBI (셀프 서비스 BI) 워크 로드에서 업무상 중요 한 작업을 분리 하기 위해 두 개의 용량을 만들 수 있습니다. 업무상 중요 한 용량을 사용 하 여 IT 부서 에게만 부여 된 작성 액세스와 함께 보장 된 리소스를 제공 하는 대기업 모델을 격리할 수 있습니다. SSBI 용량을 사용 하면 비즈니스 분석가에 게 액세스 권한을 부여 하 여 점점 더 많은 작은 모델을 호스트할 수 있습니다. SSBI 용량은 때때로 쿼리 또는 새로 고침 지속할 될 수 있습니다.

시간이 지남에 따라 용량 관리자는 작업 영역 간에 콘텐츠를 이동 하거나 용량 간에 작업 영역을 확장 하거나 용량을 확장 하거나 축소 하 여 작업 영역 간에 균형을 맞출 수 있습니다. 일반적으로 확장 하는 대규모 모델을 호스트 하 고 더 높은 동시성을 위해 규모 확장 합니다.

라이선스 구매는 v 코어를 사용 하 여 테 넌 트를 제공 합니다. **P3** 구독의 구매를 사용 하 여 하나 또는 최대 4 개의 프리미엄 용량 (예: 1 x P3, 2 x P2 또는 4 x P1)을 만들 수 있습니다. 또한, P2 용량을 P3 용량으로 업사이징 하기 전에 v 코어를 분할 하 여 두 개의 P1 용량을 만들도록 고려 사항을 제공할 수 있습니다.

### <a name="testing-approaches"></a>테스트 방법

용량 크기를 결정 한 후에는 제어 된 환경을 만들어 테스트를 수행할 수 있습니다. 실질적으로 경제적인 옵션은 Azure (Sku) 용량을 만드는 것입니다 .이 경우 P1 용량은 A4 용량과 동일한 크기 이며 P2 및 P3 용량은 각각 A5 및 A6 용량과 동일한 크기를 가집니다. Azure 용량은 빠르게 만들 수 있으며 시간 단위로 요금이 청구 됩니다. 따라서 테스트가 완료 되 면 발생 비용을 중지 하기 위해 쉽게 삭제할 수 있습니다.

Azure 용량에 생성 된 작업 영역에 테스트 콘텐츠를 추가 하 고 단일 사용자가 보고서를 실행 하 여 현실적인 쿼리 워크 로드를 생성할 수 있습니다. 가져오기 모델이 있는 경우 각 모델에 대 한 새로 고침을 수행 해야 합니다. 그런 다음 모니터링 도구를 사용 하 여 리소스 사용률을 파악 하는 모든 메트릭을 검토할 수 있습니다.

테스트를 반복 하는 것이 중요 합니다. 테스트는 여러 번 실행 되어야 하며 매번 거의 동일한 결과를 제공 해야 합니다. 이러한 결과의 평균을 사용 하 여 실제 프로덕션 조건에서 작업을 추정 하 고 예측할 수 있습니다.

부하 테스트에 사용할 용량과 보고서가 이미 있는 경우 [PowerShell 부하 생성 도구](https://aka.ms/PowerBILoadTestingTool) 를 사용 하 여 부하 테스트를 신속 하 게 생성 합니다. 이 도구를 사용 하면 한 시간에 실행 될 수 있는 각 보고서의 인스턴스 수를 예측할 수 있습니다. 도구를 사용 하 여 개별 보고서 렌더링에 대 한 용량을 평가 하거나 여러 보고서를 병렬로 렌더링할 수 있습니다. 자세한 내용은 Microsoft Power BI 비디오 [를 참조 하세요. 프리미엄 용량](https://www.youtube.com/watch?time_continue=1860&v=C6vk6wk9dcw).

더 복잡 한 테스트를 생성 하려면 실제 워크 로드를 시뮬레이트하는 부하 테스트 응용 프로그램을 개발 하는 것이 좋습니다. 자세한 내용은 웹 세미나 [Load 테스트 Power BI 응용 프로그램을 사용 하 여 Visual Studio 부하 테스트](https://blogs.msdn.microsoft.com/charles_sterling/2018/04/04/webinar-load-testing-power-bi-applications-with-visual-studio-load-test/)를 참조 하세요.
...
- 새로 고쳐지지 않고 새로 고침을 수행 하기 전에 메모리에 없는 데이터 집합 이지만, 많은 새로 고침 시간 동안 대화형 트래픽을 표시 하기 시작 했습니다. 언제 든 지 메모리에 로드 된 데이터 집합을 확인 하기 위해 Power BI 관리자는 앱의 데이터 **집합 탭의** 데이터 집합 영역을 확인 하 고 **매시간 로드 된 데이터 집합**수의 막대 중 하나를 클릭 하 여 지정 된 시간으로 교차 필터링 할 수 있습니다. 로컬 스파이크 (아래 이미지에 표시 됨)는 여러 데이터 집합이 메모리로 로드 된 시간을 나타내며,이로 인해 예약 된 새로 고침의 시작이 지연 될 수 있습니다.
...
작업 부하를 완료 하기 위한 용량이 충분 한 메모리가 있는지 확인 하기 위해 Power BI 관리자는 앱의 **데이터 집합** 탭에서 **사용 된 메모리 백분율** 시각적 개체를 참조할 수 있습니다. **모두** (전체) 메모리는 현재 쿼리 또는 처리 여부에 관계 없이 메모리에 로드 된 데이터 집합에서 사용 되는 메모리를 나타냅니다. **활성** 메모리는 현재 처리 중인 데이터 집합에 사용 되는 메모리를 나타냅니다.
...
<a name="endnote-01"></a>\[1\] 이 기술 문서에서는 Power BI 클라우드 서비스 에서만 지원 되는 Power BI Premium에 대해 설명 하므로 Power BI Report Server를 설치 하는 데 필요한 라이선스를 일부에 포함 하는 것을 제외 하 고 Power BI Report Server 범위 내에 있지 않습니다. Sku를 Power BI Premium 합니다.

<a name="endnote-02"></a>\[2\] 응용 프로그램 사용자를 대신 하 여 콘텐츠를 포함 하는 데 사용 되는 클라우드 서비스로 Power BI는 PaaS (Platform as a service)입니다. 이러한 유형의 포함은 다른 두 제품을 사용 하 여 구현할 수 있으며, 그 중 하나는 Power BI Premium입니다.

<a name="endnote-03"></a>\[3\] 푸시, 스트리밍 및 하이브리드 데이터 집합은 프리미엄 용량에 저장 되지 않으므로 프리미엄 용량을 배포, 관리 및 모니터링할 때 고려 되지 않습니다.

<a name="endnote-04"></a>\[4\] Power BI 콘텐츠 형식으로 된 Excel 통합 문서는 프리미엄 용량에 저장 되지 않으므로 프리미엄 용량을 배포, 관리 또는 모니터링할 때 고려 되지 않습니다.

<a name="endnote-05"></a>\[슬라이서\] 상호 작용을 무시 하도록 시각적 개체 5 개를 구성할 수 있습니다. 자세한 내용은 [Power BI 보고서 문서의 시각화 상호 작용](service-reports-visual-interactions.md) 을 참조 하세요.

<a name="endnote-06"></a>\[6\] 크기 차이는 파일에 대해를 사용 하 여 Power BI Desktop 파일 크기를 작업 관리자 메모리와 비교 하 여 확인할 수 있습니다.

<a name="endnote-07"></a>\[7\] Microsoft 데이터 원본에 대 한 지원에는 SQL Server, Azure data brick, Azure HDInsight Spark (베타), Azure SQL Database 및 Azure SQL Data Warehouse가 있습니다. 추가 소스에 대 한 자세한 내용은 Power BI 문서 [에서 직접 쿼리를 통해 지원 되는 데이터 원본](desktop-directquery-data-sources.md) 을 참조 하세요.

<a name="endnote-08"></a>\[8\] Power BI Premium 최대 10gb 크기의 Power BI Desktop (.pbix) 파일 업로드를 지원 합니다. 업로드 되 면 데이터 집합은 새로 고침의 결과로 크기가 최대 12gb까지 증가할 수 있습니다. 최대 업로드 크기는 SKU에 따라 다릅니다. 자세한 내용은 [대량 데이터 집합에 대 한 Power BI Premium 지원](service-premium-large-datasets.md) 문서를 참조 하세요.

<a name="endnote-09"></a>\[4 개 미만의 v 코어를 포함 하는sku는전용인프라에서실행되지않습니다.\] 여기에는 EM1, EM2, A1 및 A2 Sku가 포함 됩니다.

<a name="endnote-10"></a>\[10\] 드물지만 서비스 작업으로 인해 메모리에서 모델이 언로드될 수 있습니다.

<a name="endnote-11"></a>\[11\] 이러한 타이밍은 언제 든 지 변경 될 수 있습니다.

<a name="endnote-12"></a>\[12\] 이는 현재 미리 보기로 제공 되는 다중 지역 이라고 합니다. 다중 지역 배포에 대 한 이론적 원리는 일반적으로 성능 및 확장성이 아니라 회사 또는 정부 규정 준수를 위한 것입니다. 보고서 및 대시보드 로드에는 여전히 메타 데이터의 홈 지역에 대 한 요청이 포함 됩니다. 자세한 내용은 [Power BI Premium (미리 보기)에 대 한 다중 지역 지원](service-admin-premium-multi-geo.md) 문서를 참조 하세요.

<a name="endnote-13"></a>\[13\] 사용자가 작업으로 Power BI 서비스을 오버 로드 하 고 과도 하 게 복잡 한 쿼리를 작성 하 고 순환 참조를 만들어 성능 문제를 일으킬 수 있습니다.

<a name="endnote-14"></a>\[14\] 전체 조직의 작업 영역을 할당 하는 옵션을 사용 하지 않는 것이 좋습니다 .이 옵션을 사용 하는 것이 좋습니다. 일반적으로 프로덕션 콘텐츠에는 개인 작업 영역을 사용 하지 않는 것이 좋습니다.

<a name="endnote-15"></a>\[15\] 앱 또는 Azure Portal에서 sku를 모니터링할 수는 있지만 Power BI 관리 포털에서는 모니터링할 수 없습니다. Sku를 모니터링 하려면 앱이 리소스의 판독기 역할에 추가 되지 않은 경우 보고서 새로 고침이 실패 합니다. 자세한 내용은 [모니터 Power BI Premium 및 Power BI Embedded 용량](service-admin-premium-monitor-capacity.md) 문서를 참조 하세요.

<a name="endnote-16"></a>\[16\] 새로 고침은 CPU 또는 메모리가 부족 하 여 시작할 수 없을 때까지 대기할 수 있습니다.

<a name="endnote-17"></a>\[17\] 메모리의 데이터 집합 크기는 디스크의 크기 보다 최대 20%까지 클 수 있습니다.

<a name="endnote-18"></a>\[18\] 평균 메모리 사용량 (gb) 및 최고 메모리 사용량 (gb)

<a name="endnote-19"></a>\[19\] 데이터 집합 제거

<a name="endnote-20"></a>\[20\] 개의 데이터 집합 쿼리, 데이터 집합 평균 쿼리 기간 (밀리초), 데이터 집합 대기 수 및 데이터 집합 평균 대기 시간 (밀리초)

<a name="endnote-21"></a>\[21\] cpu 사용률이 가장 높은 사용률 (지난 7 일)의 cpu 시간 및 cpu 시간

<a name="endnote-22"></a>\[22\] 개 DQ/lc 높은 사용률 수 및 최고 사용률의 dq/lc 시간 (지난 7 일)