...
|                     | EM3                      | P1                       | P2                      | P3                       |
|---------------------|--------------------------|--------------------------|-------------------------|--------------------------|
| 페이지가 매겨진 보고서 | 해당 사항 없음 | 기본값 20%, 최솟값 10% | 기본값 20%, 최솟값 5% | 기본값 20%, 최솟값 2.5% |
| 데이터 흐름 | 기본값 20%, 최솟값 8%  | 기본값 20%, 최솟값 4%  | 기본값 20%, 최솟값 2% | 기본값 20%, 최솟값 1%  |
| AI | N/A | 기본값 20%, 최소값 20%  | 기본값 20%, 최솟값 10% | 기본값 20%, 최솟값 5%  |
| | | | | |
...
- **동시 활성 모델** : 서로 다른 가져오기 모델을 동시에 쿼리 하는 경우 메모리에 유지 되 면 최상의 응답성과 성능을 제공 합니다. 자주 쿼리되는 모든 모델을 호스트하는 데 충분한 메모리와 새로 고침에 사용할 추가 메모리가 있어야 합니다.
...
- **동시 쿼리** : 프로세서 또는 LC/DQ 연결이 용량 제한을 초과 하는 경우 많은 동시 쿼리를 실행 하면 보고서가 응답 하지 않을 수 있습니다. 이 문제는 특히 많은 시각적 개체를 포함하는 보고서 페이지에서 발생합니다.
...
테스트를 반복 하는 것이 중요 합니다. 테스트를 여러 번 실행해야 하며, 매번 거의 동일한 결과를 제공해야 합니다. 이러한 결과의 평균을 사용하여 실제 프로덕션 조건에서의 워크로드를 추정하고 예측할 수 있습니다.
