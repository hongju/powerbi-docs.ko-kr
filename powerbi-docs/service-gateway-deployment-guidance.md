...
로컬 컴퓨터의 모든 로그 파일을 볼 수 있는 쉬운 방법은 초기 설치가 완료된 후 게이트웨이를 다시 열 때 **온-프레미스-데이터-게이트웨이의 *로그 내보내기* 단추를 사용한 다음 **진단 > 로그 내보내기**를 선택하는 것입니다.
...
**Power BI 서비스**에서 게이트웨이 관리자 목록을 찾을 수 있습니다. **Power BI 서비스에 로그인 할 때 **설정**(기어 아이콘) -> "게이트웨이 관리" -> 게이트웨이 UI를 선택합니다.  
