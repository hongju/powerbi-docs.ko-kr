...
아래 이미지는 "(3,10s]" 실행 기간 버킷의 크기로 표시되고, 데이터 세트의 성능이 크게 저하된 1월 30일의 1시간을 표시합니다. 해당 1시간 막대를 클릭하면 해당 시간 동안 메모리에 로드된 모든 데이터 세트를 표시하며, 시끄러운 이웃 효과 발생이 가능한 데이터 세트를 표시합니다.
...
일반적으로 용량의 평균 CPU 사용률은 80% 이하로 유지되어야 합니다. 이 값을 초과하는 것은 용량이 CPU 포화에 근접하고 있다는 것을 의미합니다.

CPU 포화는 모든 작업을 처리하려고 시도할 때 많은 CPU 컨텍스트 스위치를 수행하는 용량으로 인해 필요한 시간보다 오래 걸리는 작업으로 알 수 있습니다. 많은 수의 동시 쿼리를 사용하는 프리미엄 용량에서는 긴 쿼리 대기 시간으로 이를 알 수 있습니다. 쿼리 대기 시간이 길어지면 평소 보다 느린 응답을 얻게 됩니다. Power BI 관리자는 **시간별 쿼리 대기 시간 분포** 시각적 개체를 확인하여 CPU가 포화 상태일 때 쉽게 식별할 수 있습니다. 쿼리 대기 시간 개수가 주기적인 피크를 보이는 것은 CPU 포화 가능성이 있음을 시사합니다.

![정기적인 최대 쿼리 대기 시간 잠재적인 CPU 포화를 나타낼 수](media/service-premium-capacity-scenarios/peak-query-wait-times.png)

CPU 포화에 영향을 주는 경우 백그라운드 작업에서 비슷한 패턴이 때때로 감지될 수 있습니다. Power BI 관리자는 이 때(아마도 다른 지속적인 데이터 세트 새로 고침 및/또는 대화형 쿼리 때문에) CPU 포화를 의미할 수 있는 특정 데이터 세트의 새로 고침 시간의 정기적인 급증을 찾아볼 수 있습니다. 이 인스턴스에서, 앱의 **시스템** 보기를 참조하면 CPU가 반드시 100%로 표시되지 않을 수도 있습니다. **시스템** 보기는 시간당 평균을 표시하지만 대기 시간이 급증을 보이는 몇 분 동안의 과도한 작업으로 CPU는 포화될 수 있습니다.

CPU 포화의 효과에는 더 많은 미묘한 사항들이 있습니다. 대기 쿼리 수는 중요하지만, 쿼리 대기 시간은 눈에 띄는 성능 저하 없이 항상 어느 정도 수준까지는 발생합니다. 일부 데이터 세트(복잡하거나 크기가 커서 평균 쿼리 시간이 긴 데이터 세트)는 다른 데이터 세트보다 CPU 포화의 영향을 더 쉽게 받을 수 있습니다. 이러한 데이터 세트를 쉽게 식별하려면, Power BI 관리자는 **시간별 대기 시간 분포** 시각적 개체의 막대의 색 구성의 변화를 살펴볼 수 있습니다. 특이치 막대를 찾은 후, 이 시간 동안 쿼리가 대기했던 데이터 세트를 확인할 수 있으며 또한 평균 쿼리 기간과 평균 쿼리 대기 시간을 비교해서 살펴볼 수도 있습니다. 두 메트릭의 크기가 동일하고 데이터 세트에 대한 쿼리 워크로드가 사소하지 않은 경우, 데이터 세트는 불충분한 CPU의 영향을 받고 있을 가능성이 있습니다.

이러한 효과는 데이터 세트를 여러 사용자에 의해 자주 쿼리하는 짧은 버스트에서 소비하여 각 버스트 동안 CPU 포화가 발생할 때 특히 뚜렷하게 나타날 수 있습니다(예를 들어, 교육 세션의 경우). 이 경우, 용량의 다른 데이터 세트에도 영향을 주는 것(시끄러운 이웃 효과) 뿐만 아니라 이 데이터 세트에서 쿼리 대기 시간이 크게 길어질 수 있습니다.

일부 경우, Power BI 관리자는 데이터 세트 소유자가 보고서 대신 대시보드(캐시된 타일에 대한 데이터 세트 새로 고침으로 주기적으로 쿼리)를 생성하여 휘발성이 적은 쿼리 워크로드를 생성하도록 요청할 수 있습니다. 이것은 대시보드를 로드할 때 급증을 방지하는 데 도움이 될 수 있습니다. 이 솔루션은 특정 비즈니스 요구 사항에 대해 항상 가능한 것은 아니지만, 데이터 세트를 변경하지 않고 CPU 포화를 방지하는 효과적인 방법일 수 있습니다.
