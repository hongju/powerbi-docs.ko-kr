...
일반적으로 용량의 평균 CPU 사용률은 80% 이하로 유지 되어야 합니다. 이 값을 초과하는 것은 용량이 CPU 포화에 근접하고 있다는 것을 의미합니다.

CPU 포화의 효과는 모든 작업을 처리 하려고 시도할 때 많은 CPU 컨텍스트 스위치를 수행 하는 용량으로 인해 필요한 시간보다 오래 걸리는 작업에 의해 표현됩니다. 많은 수의 동시 쿼리를 사용하는 프리미엄 용량에서는 높은 쿼리 대기 시간으로 표시됩니다. 높은 쿼리 대기 시간의 결과는 평소 보다 느린 응답입니다. Power BI 관리자는 **시간별 쿼리 대기 시간 분포** 시각적 개체를 확인하여 CPU 포화 상태일 때 쉽게 식별할 수 있습니다. 쿼리 대기 시간 개수의 주기적인 피크는 잠재적인 CPU 포화를 나타냅니다.

![정기적인 최대 쿼리 대기 시간 잠재적인 CPU 포화를 나타낼 수](media/service-premium-capacity-scenarios/peak-query-wait-times.png)

CPU 포화에 영향을 주는 경우 백그라운드 작업에서 비슷한 패턴이 때때로 감지 될 수 있습니다. Power BI 관리자는 그 시점에(아마도 다른 지속적인 데이터 집합 새로 고침 및/또는 대화형 쿼리 때문에) CPU 포화를 나타낼 수 있는 특정 데이터 세트의 새로 고침 시간에 정기적으로 급증을 찾을 수 있습니다. 이 인스턴스에서, 앱의 **시스템** 보기를 참조하면 CPU가 반드시 100%로 표시되지 않을 수도 있습니다. **시스템** 보기는 시간당 평균을 표시하지만 대기 시간이 급증으로 나타나는 몇 분 동안 과도한 작업으로 CPU는 포화 될 수 있습니다.

CPU 포화의 효과를 보는 것에는 더 많은 미묘한 차이가 있습니다. 대기하는 쿼리 수는 중요하지만, 쿼리 대기 시간은 눈에 띄는 성능 저하 없이 항상 어느 정도 수준까지 발생합니다. 일부 데이터 세트(평균 쿼리 시간이 길수록 복잡성 또는 크기를 나타냄)는 다른 데이터 세트 보다 CPU 포화의 영향을 더 쉽게 받을 수 있습니다.이러한 데이터 세트를 쉽게 식별 하려면, Power BI 관리자는 **시간별 대기 시간 분포** 시각적 개체의 막대의 색 구성에서 변경 내용을 볼 수 있습니다. 특이치 막대를 찾은 후, 이 시간 동안 쿼리가 대기했던 데이터 세트를 확인할 수 있으며 또한 평균 쿼리 기간에 비해 평균 쿼리 대기 시간을 비교해서 살펴볼 수도 있습니다. 두 메트릭의 크기가 동일하고 데이터 세트에 대한 쿼리 워크로드가 중요하지 않은 경우, 데이터 세트가 불충분한 CPU의 영향을 받을 가능성이 있습니다.

이러한 효과는 데이터 세트를 여러 사용자에 의해 자주 쿼리하는 짧은 버스트에서 소비하여 각 버스트 동안 CPU 포화가 발생할 때 특히 뚜렷하게 나타날 수 있습니다(예를 들어, 교육 세션의 경우). 이 경우, 용량의 다른 데이터 세트(시끄러운 이웃 효과)에도 영향을 주는 것뿐만 아니라 이 데이터 세트에서 중요한 쿼리 대기 시간을 경험할 수 있습니다.

어떤 경우, Power BI 관리자는 데이터 세트 소유자가 보고서 대신 대시보드(캐시된 타일에 대한 데이터 세트 새로 고침으로 주기적으로 쿼리)를 생성하여 휘발성이 적은 쿼리 워크로드를 생성하도록 요청할 수 있습니다. 이것은 대시보드를 로드할 때 급증을 방지하는데 도움이 될 수 있습니다. 이 솔루션은 특정 비즈니스 요구 사항에 대해 항상 가능한 것은 아니지만, 데이터 세트를 변경하지 않고 CPU 포화를 방지 하는 효과적인 방법 일 수 있습니다.
