...
관계를 기반으로 하는 **집계**는 일반적으로 차원 모델과 함께 사용됩니다. 데이터 웨어하우스 및 데이터 마트에서 제공하는 Power BI 데이터 세트는 차원 테이블과 팩트 테이블 간의 관계가 있는 스타 스키마(star schema) 및 스노우 플레이크 스키마(snowflake schema)와 비슷합니다.
