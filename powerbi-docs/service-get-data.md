...
**서비스** – Power BI용 콘텐츠 팩이 있는 여러 가지 서비스의가 있으며 이후에도 계속 추가됩니다. 대부분의 서비스에는 계정이 있어야 합니다. 자세한 내용은 [서비스에 연결](service-connect-to-services.md)을 참조하세요.
...
"하지만 해당 데이터베이스의 물류 데이터는 매일 변경되는데, Power BI의 데이터 집합이 새로 고쳐지는지 어떻게 확인하나요?"라고 생각할 수도 있습니다.” Power BI Desktop 또는 Excel 파일에서 연결 정보는 데이터와 함께 데이터 집합으로 가져옵니다. 예정된 새로 고침을 설정하거나 데이터 집합에 대한 수동 새로 고침을 수행하면 Power BI는 몇 가지 다른 설정과 함께 데이터 집합에서 연결 정보를 사용하여 데이터 베이스에 직접 연결하고 업데이트를 쿼리하며 해당 업데이트를 데이터베이스에 직접 로드합니다. Power BI 게이트웨이는 온-프레미스 서버 및 Power BI 간에 데이터 전송을 보호하기 위해 필요합니다. 보고서 및 대시보드의 시각화는 자동으로 새로 고쳐집니다.

Power BI 서비스에서 바로 데이터 원본에 연결할 수 없다고 해서 PowerBI로 해당 데이터를 가져올 수 없는 것은 아닙니다. 몇 가지 단계가 더 걸릴 수 있으며 IT 부서의 도움이 필요할 수 있습니다. 자세히 알아보려면 [Power BI Desktop의 데이터 원본](desktop-data-sources.md)을 참조하세요.
