...
새 작업 영역은 4개의 역할(관리자, 구성원, 참가자 및 뷰어)을 제공합니다.
...
작업 영역이 Power BI Premium 용량에 있으면 뷰어 역할을 가진 사용자는 Power BI Pro 라이선스가 없어도 작업 영역에 액세스할 수 있습니다. 그러나 이 사용자에게 관리자, 구성원 또는 참가자와 같은 상위 역할을 할당하면 해당 사용자는 작업 영역에 액세스할 수 없습니다. 작업 영역에 액세스하려고 하면 Pro 평가판을 시작할지 묻는 메시지가 표시됩니다. Pro 라이선스가 없는 사용자에 대해 뷰어 기능을 이용하려면 뷰어 역할의 사용자가 개별적으로 또는 사용자 그룹을 통해 다른 작업 영역에 포함되지 않는지 확인합니다. 
