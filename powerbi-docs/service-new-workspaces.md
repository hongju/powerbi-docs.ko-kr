...
- 뷰어 역할이 있는 사용자가 데이터를 내보낼 수 있도록 하려면 작업 영역의 데이터 세트에 대한 빌드 권한이 있는지 확인합니다. [데이터 세트 빌드 권한](service-datasets-build-permissions.md#build-permissions-for-shared-datasets)에 대해 자세히 알아보세요.
- **작업 영역 나가기** 단추가 없습니다.
