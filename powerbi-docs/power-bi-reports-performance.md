...
일반적인 실수는 테이블의 기본 보기, 즉 1억 개 이상의 모든 행을 필터링하지 않도록 하는 것입니다. 이러한 행에 대한 데이터는 메모리로 로드되고 새로 고칠 때마다 압축을 풀어야 합니다. 이를 통해 거대한 메모리 로드가 생성됩니다. 해결책은 테이블이 “Top N” 필터를 사용하여 표시되는 항목의 최대 수를 줄이는 것입니다. 최대 항목은 10,000개와 같이 사용자가 필요로 하는 것보다 훨씬 더 많을 수 있습니다. 결과적으로, 최종 사용자 환경은 변경되지 않았지만 보고서의 메모리 사용률은 여러 작업량을 떨어뜨렸고 이에 따라 성능이 향상되었습니다.
...
DirectQuery 또는 라이브 연결 사례에서 사용자가 Power BI 보고서를 방문하는 경우 Power BI에서 쿼리를 실시간으로 기본 데이터 원본으로 보냅니다. 데이터 원본이 쿼리 데이터와 함께 반환되고 나면 보고서가 렌더링됩니다. 결과적으로, 이러한 경우의 보고서 성과는 기본 데이터 원본의 성능에 크게 달라집니다.
...
- 장기 실행 쿼리가 시스템 리소스를 독점할 수 없도록 서비스에서 적용하는 쿼리 수준 시간 제한이 있습니다. 쿼리가 225초보다 길어지면 시간이 초과되며 시각적 개체 수준 오류가 발생합니다.
...
SQL 프로파일러를 통해 가장 긴 CPU 시간을 차지하고, 이에 따라 성능 병목 상태가 발생할 가능성이 높은 쿼리를 식별할 수 있습니다. 이러한 쿼리를 실행하는 시각적 개체는 지속적인 최적화의 초점이 되어야 합니다.
...
네트워크 대기 시간은 Power BI 서비스 연결에 대한 요청 및 전달할 응답에 필요한 시간이 늘어나면 보고서 성능에 영향을 줄 수 있습니다. Power BI의 테넌트는 특정 지역에 할당됩니다. powerbi.com으로 이동하고, 오른쪽 위에서 ?**를 선택한 다음, **Power BI 정보**를 선택하면 테넌트의 "홈" 지역을 볼 수 있습니다. 테넌트의 사용자가 Power BI 서비스에 액세스하는 경우 해당 요청은 항상 이 영역에 라우팅됩니다. 요청이 Power BI 서비스에 도달하면 서비스에서 추가 요청을 기본 데이터 원본 또는 게이트웨이와 같은 대상으로 보낼 수 있습니다. 또한 네트워크 대기 시간의 영향도 받습니다.
