...
시각적 개체가 표시해야 하는 데이터가 많으면 많을수록 시각적 개체는 더 느리게 로드됩니다. 이 원칙은 명확해 보이지만 잊어버리기 쉬울 수 있습니다. 예를 들어 큰 데이터 집합이 있다고 가정합니다. 맨 처음에 테이블의 테이블을 사용하여 리포트를 빌드합니다. 최종 사용자는 페이지에서 슬라이서를 사용하여 원하는 행으로 이동합니다. 일반적으로 수십 개 행에만 관심이 있습니다.

일반적인 실수는 1억 개 이상의 모든 행과 같이 필터링되지 않은 테이블의 기본 보기가 있는 것입니다. 이러한 행에 대한 데이터는 메모리로 로드되고 새로 고칠 때마다 압축을 풀어야 합니다. 이를 통해 거대한 메모리 로드가 생성됩니다. 해결책은 테이블이 “Top N” 필터를 사용하여 표시되는 항목의 최대 수를 줄이는 것입니다. 최대 항목은 10,000개와 같이 사용자가 필요로 하는 것보다 훨씬 더 많을 수 있습니다. 결과적으로, 최종 사용자 환경이 변경되지 않고 보고서의 메모리 사용률이 복수 자릿수와 그에 맞춰 향상된 성능이 떨어집니다. 
 
보고서의 모든 시각적 개체에 위와 유사한 방법을 적용해야 합니다. 스스로에게 물어보세요. 이 시각적 개체의 모든 데이터가 정말로 필요한가? 최종 사용자 환경에 끼치는 영향이 최소로 하면서 시각적 개체에 표시되는 데이터 크기를 필터링하는 방법이 있는가? 특히 테이블에는 비용이 매우 많이 들 수 있습니다. 
...
- 사용하지 않은 테이블 또는 열은 가능한 경우 제거해야 합니다. 
- 수백만의 고유 값과 같이 카디널리티가 높은 필드에서 고유 카운트를 사용하지 않습니다.  
- 불필요한 정밀도 및 높은 카디널리티가 있는 필드를 사용하지 않는 단계를 수행합니다. 예를 들어 고유 날짜/시간 값을 월, 연도, 날짜 등과 같이 개별 열로 분할할 수 있습니다. 또는 가능한 경우 고정밀 필드에서 반올림을 사용하여 카디널리티를 줄입니다(예: 13.29889 -> 13.3). 
- 가능한 경우 문자열 대신 정수를 사용합니다. 
- 테이블의 모든 행을 테스트하는 데 필요한 DAX 함수(예: RANKX)를 주의해야 합니다. 최악의 경우 이러한 함수는 테이블 크기에서 선형 증가가 정해진 런타임 및 메모리 요구 사항을 기하급수적으로 늘릴 수 있습니다. 
- DirectQuery를 통해 데이터 원본에 연결할 때 일반적으로 필터링되고 다시 조각화되는 열의 인덱싱을 고려합니다. 그러면 보고서 응답성이 대폭 향상됩니다.  
 

DirectQuery를 위한 데이터 원본 최적화에 대한 자세한 지침은 [SQL Server 2016 Analysis Services의 DirectQuery](https://blogs.msdn.microsoft.com/analysisservices/2017/04/06/directquery-in-sql-server-2016-analysis-services-whitepaper/)를 참조하세요. 
...
- 관계에서 “참조 무결성 가정”을 설정해 봅니다. 대부분의 경우에서 쿼리 성능이 크게 향상될 수 있습니다.  
...
- 서비스에서는 장기 실행 쿼리가 시스템 리소스를 독점할 수 없도록 쿼리 수준 시간 제한을 강제 적용합니다. 쿼리가 225초보다 길어지면 시간이 초과되며 시각적 개체 수준 오류가 발생합니다. 

## <a name="understanding-dashboards-and-query-caches"></a>대시보드 및 쿼리 캐시의 이해 
...
DirectQuery 및 라이브 연결의 경우 이 쿼리 캐시는 데이터 원본을 쿼리하여 주기적으로 업데이트됩니다. 기본적으로 1시간마다 발생하며, 데이터 집합 설정에서 구성할 수 있습니다. 각 쿼리 캐시 업데이트는 쿼리를 데이터 원본에 전송하여 캐시를 업데이트합니다. 생성된 쿼리의 수는 해당 데이터 원본에 의존하는 대시보드에 고정된 시각적 개체의 수에 따라 달라집니다. 행 수준 보안을 사용하는 경우 각기 다른 보안 컨텍스트에 대해 쿼리가 생성됩니다. 예를 들어 사용자가 속한 두 가지 다른 역할이 있는 경우 데이터의 두 가지 다른 보기를 사용하면 쿼리 캐시를 새로 고치는 동안 쿼리의 두 가지 집합이 생성됩니다. 
...
2. **Power BI Desktop에서 사용 중인 포트 파악** 
   ...
   관리자 권한으로 명령 프롬프트나 PowerShell을 실행하고 netstat를 사용하여 Power BI Desktop에서 분석을 위해 사용 중인 포트를 찾습니다.
...
SQL 프로파일러를 통해 가장 긴 CPU 시간을 차지하는, 그래서 성능 병목 현상으로 이어질 가능성이 높은 쿼리를 식별할 수 있습니다. 이러한 쿼리를 실행하는 시각적 개체는 지속적인 최적화의 초점이 되어야 합니다. 

## <a name="gateway-best-practices"></a>게이트웨이 모범 사례 

온-프레미스 데이터 게이트웨이는 온-프레미스 데이터를 사용한 Power BI 서비스를 연결하기 위한 좋은 도구입니다. 동시에 계획이 부실하면 보고서 성능에서 병목 현상이 일어날 수 있습니다. 이는 모든 쿼리 및 쿼리 응답이 게이트웨이를 통해 전달되는 DirectQuery/라이브 연결 데이터 집합에서는 특히 그러합니다. 다음은 고성능 게이트웨이를 보장하기 위한 몇 가지 모범 사례입니다. 
 
- 개인 모드에 반대로 **엔터프라이즈 모드를 사용**합니다. 
- **게이트웨이에 권장되는 하드웨어 사양** - 8 CPU 코어, 16GB RAM. 
...
- **수직 또는 수평 확장** - 게이트웨이가 실제로 병목 상태가 되는 경우, 수직 확장(즉, 더 많은 CPU와 RAM을 사용하는 더 강력한 컴퓨터로 게이트웨이를 이동) 또는 수평 확장(예: 다른 게이트웨이로 데이터 집합을 분할)을 고려합니다. 
