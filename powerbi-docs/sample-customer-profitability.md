...
이 샘플은 비즈니스 중심 데이터, 보고서 및 대시보드가 포함된 Power BI를 사용하는 방법을 보여 주는 시리즈의 일부입니다. 이 샘플은 obviEnce([www.obvience.com](http://www.obvience.com/))의 실제 데이터로 작성되고, 익명으로 처리되었습니다. 데이터는 콘텐츠 팩/앱, Excel 통합 문서 또는 .pbix Power BI Desktop 파일 등 여러 형식으로 사용할 수 있습니다. [Power BI용 샘플](sample-datasets.md)을 참조하세요.

## <a name="prerequisites"></a>필수 조건
함께 진행해볼까요? 이 자습서에서는 Power BI 서비스 및 고객 수익성 샘플 콘텐츠 팩을 사용합니다. 보고서 환경은 비슷하므로 Power BI Desktop과 샘플 .pbix 파일을 사용하여 따라 할 수도 있습니다. 콘텐츠 팩 및 .pbix 파일에 연결하는 지침은 다음 섹션에서 설명합니다.
...
또는 [고객 수익성 샘플](http://download.microsoft.com/download/6/A/9/6A93FD6E-CBA5-40BD-B42E-4DCAE8CDD059/Customer%20Profitability%20Sample%20PBIX.pbix)을 Power BI Desktop에서 사용하도록 설계된 .pbix 파일로 다운로드할 수 있습니다.
...
3. Carlos가 아직 목표를 달성하지 못한 이유를 살펴보겠습니다.  
    ...
    b. 첫 번째 피벗 테이블을 보면, Carlos의 상위 제품인 Primus에 대한 수익 성장이 작년보다 152% 감소했습니다. 그리고 전년 대비 수익 분산 차트에 따르면, 대부분의 달에 예산을 밑돌았습니다.  
...
팀 멤버 중 두 명에 주목하여 어떤 통찰력을 얻을 수 있는지 보겠습니다. 왼쪽 **경영진** 상자에서 Andrew의 이름을 선택하여 Andrew에 대한 데이터만 표시하도록 보고서 페이지를 필터링합니다.
...
이제 Valery를 봅시다. **경영진** 상자에서 Valery의 이름을 선택하여 그녀에 대한 데이터만 표시하도록 보고서 페이지를 필터링합니다. 
...
Valery의 영역에서 실적이 저조한 이유를 계속 살펴봅시다. 지역, 다른 사업부 및 보고서의 다음 페이지: **업계 수익 분석**을 살펴봅니다.
...
업계별로 수익성이 어떻게 달라지는가? 제품 및 고객을 어떻게 업계 별로 나누는가? 이러한 질문에 답하려면 왼쪽 상단에서 하나 이상의 업계를 선택합니다(CPG 업계에서 시작). 필터를 지우려면 지우개 아이콘을 선택합니다.

거품형 차트(**예산 대비 수익 VAR %, GM% 및 업계별 RevenueTY**)에서 CFO는 가장 큰 거품을 찾습니다. 이것이 수익에 가장 큰 영향을 주기 때문입니다. 업계 세그먼트별로 각 관리자의 영향을 쉽게 보려면, 영역 차트에서 각 관리자의 이름을 클릭하여 페이지를 필터링합니다.
...
이 페이지는 Cortana의 Cortana 답변 페이지 형식입니다. 자세한 내용은 [Cortana에 대한 사용자 지정 답변 페이지 만들기](service-cortana-answer-cards.md)를 참조하세요.
...
3. *Valery Ushakov에 대한 업계별 총 수익*을 입력합니다. 질문을 입력하면 시각화가 어떻게 업데이트되는지 확인합니다.
...
6. 실제 수익을 보려면 질문 및 답변을 사용하여 *경영진의 시나리오별 배포에 대한 총 수익*을 입력합니다.  
...
이 환경은 안전한 곳입니다. 언제든지 변경 내용을 저장하지 않도록 선택할 수 있습니다. 그러나 변경 내용을 저장하면 언제든지 이 샘플의 새 복사본에 대해 **데이터 가져오기**를 선택할 수 있습니다.
...
이 둘러보기가 Power BI 대시보드, 질문 및 답변, 보고서를 통해 고객 데이터를 파악하는 방법을 확인하는 데 도움이 되었기를 바랍니다. 이제 사용자 데이터에 연결할 차례입니다. Power BI를 사용하여 다양한 데이터 원본에 연결할 수 있습니다. [Power BI 시작하기](service-get-started.md)에 대해 자세히 알아보세요.
