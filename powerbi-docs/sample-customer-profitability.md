...
“고객 수익성 샘플” 콘텐츠 팩은 마케팅 자료를 제조하는 회사에 대한 대시보드, 보고서 및 데이터 집합을 포함합니다. 이 대시보드는 5개 비즈니스 단위 관리자(즉, 임원), 제품, 고객 및 총수익(GM)에 대한 주요 메트릭을 확인하기 위해 CFO가 만들었습니다. 수익성에 영향을 주는 팩트를 한 눈에 볼 수 있습니다.
...
[이 샘플의 데이터 집합(Excel 통합 문서)만 다운로드](http://go.microsoft.com/fwlink/?LinkId=529781)할 수도 있습니다. 통합 문서에는 보고 수정할 수 있는 파워 뷰 시트가 포함됩니다. 원시 데이터를 보려면 **파워 피벗 > 관리**를 선택합니다.
...
이 타일은 CFO가 자신에게 중요한 높은 수준의 회사 메트릭을 볼 수 있도록 해 줍니다.  관심을 끄는 무언가를 발견하면 타일을 선택하여 데이터를 상세히 확인할 수 있습니다.

1. 우리 회사의 총수익은 42.5%입니다.
2. 80명의 고객이 있습니다.
3. 5가지 제품을 판매합니다.
4. 2월에 가장 낮은 수익 분산 %를 잡았으며, 그 다음인 3월에 가장 높게 잡았습니다.
5. 수익의 대부분은 동부 및 북부 지역에서 얻습니다. 총수익은 일부 추가 조사가 필요한 ER-0 및 MA-0으로 예산을 결코 초과하지 않습니다.
6. 해당 연도의 전체 수익은 예산에 가깝습니다.

### <a name="manager-specific-dashboard-tiles"></a>관리자 전용 대시보드 타일
이 타일은 팀 성과 기록표를 제공합니다. CFO는 관리자들을 추적해야 하며, 이 타일들은 GM%를 사용하여 높은 수준의 개요를 제공합니다. GM% 추세를 어떤 관리자에게도 예상하지 않는 경우, 더 조사할 수 있습니다.

3월 이후 Annelie의 GM%가 가장 낮지만 꾸준한 증가가 보입니다. 반면 Valery의 GM%가 현저히 떨어진 것을 확인했습니다. Andrew는 불안한 한 해를 보냈습니다. 원본 보고서를 열려면 관리자 전용 대시보드 파일을 클릭합니다. 보고서는 3페이지로 되어 있으며 “업계 수익 분석” 페이지에 열립니다.

## <a name="explore-the-pages-in-the-report"></a>보고서의 페이지 탐색
보고서는 다음과 같이 3페이지로 구성되어 있습니다.
...
1. 맨 위 탐색 모음에서 **Power BI** 를 선택하여 대시보드로 돌아갑니다.
2. 대시보드의 가장 위에 있는 질문 및 답변 상자를 선택합니다.
...
1. 대시보드로 돌아가서Andrew의 총수익 추세에 대한 영역형 차트를 선택합니다. 그러면 보고서의 “업계 수익 분석” 페이지가 열립니다.
2. 보고서 페이지에서 시각화를 선택하지 않고 오른쪽 필터 창을 확대합니다. 필터 창은 페이지 수준 필터만 표시합니다.  
   
   ![](media/sample-customer-profitability/power-bi-filters.png)
3. **업계**에 대한 필터를 찾아 화살표를 선택하여 목록을 확대합니다. 유통 업계에 대한 페이지 필터를 추가해 보겠습니다. 먼저 **모두 선택** 확인란을 지워 모든 선택을 취소합니다. 그런 다음 **유통**을 선택합니다.  
...
6. 실제 수익을 보려면 대시보드로 돌아가서 질문 및 답변을 사용하여 **경영진의 시나리오별 배포에 대한 총 수익**을 문의하십시오.  
