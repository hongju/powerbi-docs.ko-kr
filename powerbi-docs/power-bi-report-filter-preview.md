...
![시각적 개체의 필터 목록](media/power-bi-report-filter-preview/power-bi-filter-visual.png)
...
2. 외쪽 탐색 모음에서 **현재 파일** 아래의 **보고서 설정**을 선택합니다.
3. **필터링 환경**에서 **업데이트된 필터 창을 사용하고 이 보고서의 시각적 머리글에 필터 표시**를 선택합니다.
...
1. 기본적으로 보고서 소비자는 필터 창을 볼 수 있습니다. 사용자가 볼 수 없도록 하려면 **필터** 옆의 눈에 보이는 아이콘을 선택합니다.
    ...
    ![Power BI 필터 눈 아이콘](media/power-bi-report-filter-preview/power-bi-filter-eye.png)

2. 새 필터 창 빌드를 시작하려면 관심 있는 필드를 시각적 개체, 페이지 또는 보고서 수준 필터로써 새 필터 창으로 끌어옵니다.

보고서 캔버스에 시각적 개체를 추가할 때 Power BI는 시각적 개체에 각 필드에 대한 필터를 필터 창에 자동으로 추가합니다. 
...
새 필터 창에서 이러한 설정을 켜거나 끄면 보고서에 반영된 변경 내용이 표시됩니다. 숨겨진 필터는 시각적 개체의 필터 팝업에 표시되지 않습니다.
...
## <a name="theming-for-filter-pane"></a>필터 창의 테마 설정
...
![필터 정렬 순서 재정렬](media/power-bi-report-filter-preview/power-bi-filter-sort.gif)
...
새 필터 창은 보고서 페이지 및 시각적 개체에 따라 확장되므로, 보고서 페이지와 필터 창이 서로 비율을 유지합니다.
...
![필터 창 접근성](media/power-bi-report-filter-preview/power-bi-filter-accessible.png)
