...
이 설정은 데이터 새로 고침 및 보고서 렌더링에 적용됩니다. 쿼리 새로 고침을 사용하지 않도록 설정할 때까지 데이터 새로 고침은 데이터 원본과 쿼리 새로 고침에서 모두 데이터를 새로 고칩니다. 쿼리 새로 고침을 사용하도록 설정한 경우 이 메모리 한도가 이러한 쿼리에도 적용됩니다. 오류가 발생하면 데이터 새로 고침이 성공했더라도 예약된 새로 고침 상태가 실패로 보고됩니다.

#### <a name="query-timeout"></a>쿼리 시간 제한

이 설정을 사용하면 사용자의 보고서가 느리게 로드되는 원인이 되는 장기 실행 쿼리를 효율적으로 제어할 수 있습니다. 이 설정은 데이터 새로 고침 및 보고서 렌더링에 적용됩니다. 쿼리 새로 고침을 사용하지 않도록 설정할 때까지 데이터 새로 고침은 데이터 원본과 쿼리 새로 고침에서 모두 데이터를 새로 고칩니다. 쿼리 새로 고침을 사용하도록 설정한 경우 이 시간 제한 한도가 이러한 쿼리에도 적용됩니다.
