...
2. 숫자가 있는 목록 열의 오른쪽에 있는 **필드 창**에 합계 또는 시그마, 아이콘이 표시되는지 확인합니다.  그렇지 않은 경우, 테이블 보기에서 열 머리글을 선택하고 **모델링** 탭을 선택한 다음, 데이터에 따라 **데이터 형식**을 **10진수** 또는 **정수**로 변경합니다.  변경 내용을 확인하라는 메시지가 표시되면 **예**를 선택합니다.  숫자가 특수 형식(예: 통화)인 경우 **서식**을 설정하여 선택할 수도 있습니다.
