...
동시에 둘 이상의 테이블을 쿼리하는 경우, 데이터가 로드될 때 Power BI Desktop은 관계를 찾고 생성하는 것을 시도합니다. 카디널리티, 교차 필터 방향 및 활성 속성이 자동으로 설정됩니다. Power BI Desktop은 쿼리하는 테이블의 열 이름을 보고 잠재적 관계가 있는지 여부를 확인합니다. 관계가 있는 경우 자동으로 생성됩니다. Power BI Desktop에서 일치하는 것이 있는지를 높은 수준의 신뢰도로 판단할 수 없는 경우, 관계를 자동으로 만들지 않습니다. 이 경우 관계 관리 대화 상자를 사용하여 관계를 만들거나 편집할 수 있습니다.
...
2. **관계 만들기** 대화 상자에서, 첫 번째 테이블 드롭다운 목록에서 테이블을 선택한 다음, 관계에 사용하려는 열을 선택합니다.
3. 두 번째 테이블 드롭다운 목록에서 관계에 사용할 다른 테이블을 선택한 다음, 사용하려는 다른 열을 선택하고 **확인**을 클릭합니다.

![](media/desktop-create-and-manage-relationships/manualrelationship2.gif)

기본적으로 Power BI Desktop은 자동으로 새 관계에 대한 카디널리티(방향), 교차 필터 방향 및 활성 속성을 구성합니다. 그러나 필요한 경우 설정을 변경할 수 있습니다. 자세한 내용은 이 문서의 뒷부분에 있는 추가 옵션 이해 섹션을 참조하세요.

관계에 대해 선택된 테이블에 고유한 값이 없는 경우 *열 중 하나는 고유한 값이어야 합니다* 라는 오류를 볼 수 있습니다. 관계에 있는 테이블 중 적어도 하나에는 모든 관계형 데이터베이스 기술에 대한 공통 요구 사항인 고유한 키 값 목록이 *있어야* 합니다. 
...
자세한 내용은 [블로그 게시물](https://blogs.technet.microsoft.com/cansql/2016/12/19/relationships-in-power-bi-fixing-one-of-the-columns-must-have-unique-values-error-message/)을 참조합니다.
...
관계를 만들거나 편집할 때 추가 옵션을 구성할 수 있습니다. 기본적으로 추가 옵션은 최상의 추측에 기반하여 자동으로 구성되며 이는 열의 데이터를 기반으로 하는 각 관계마다 다를 수 있습니다.

## <a name="cardinality"></a>카디널리티
**다대일 (\*: 1)** -한 테이블의 열은 값의 인스턴스를 하나 이상 가질 수 있는 가장 일반적인 기본 형식이며, 다른 관련된 테이블은 조회 테이블이라고 하며 값의 인스턴스가 하나만 있습니다.

**일대일 (1:1)** - 한 테이블의 열은 특정 값의 인스턴스를 하나만 가지며 다른 관련된 테이블은 하나의 특정 값 인스턴스만 가집니다.

**다 대 다 관계**: 복합 모델을 사용하여 테이블의 고유 값에 대한 요구 사항을 제거 하는 테이블 간에 다 대 다 관계를 설정할 수 있습니다. 또한 관계 설정 목적으로만 새 테이블을 도입하는 것과 같은 이전 해결 방법을 제거합니다. 자세한 내용은 [다 대 다 카디널리티와의 관계](https://docs.microsoft.com/power-bi/desktop-many-to-many-relationships)를 참조하세요. 

카디널리티를 변경하는 경우에 대한 자세한 내용은 이 아티클의 뒷부분에 있는 추가 옵션 이해 섹션을 참조하세요.

## <a name="cross-filter-direction"></a>교차 필터 방향
**모두** - 가장 일반적인 기본 방향이며, 필터링을 위해 두 테이블이 모두 단일 테이블인 것처럼 처리됩니다. **모두**는 주위에 조회 테이블이 여러 개 있는 단일 테이블에서 잘 작동합니다. 부서에 대한 조회 테이블이 있는 판매 실적 테이블을 예로 들 수 있습니다. 이를 종종 스타 스키마(Star Schema) 구성(여러 조회 테이블이 있는 중앙 테이블)이라고 합니다. 그러나 조회 테이블(일부는 공통)이 있는 테이블이 여럿인 경우 모두 설정을 사용하지 않는 것이 좋습니다. 앞의 예에서는 각 부서에 대한 목표 예산을 기록하는 판매 예산 테이블도 있습니다. 또한 부서 테이블은 판매와 예산 테이블 모두에 연결되어 있습니다. 이러한 유형의 구성에서는 모두 설정을 사용하지 않습니다.

**단일** - 연결된 테이블의 필터링 선택 항목이 값이 집계되는 테이블에서 작동합니다. Excel 2013 이전 버전의 데이터 모델에서 파워 피벗을 가져오는 경우 모든 관계는 단일 방향입니다. 
...
관계를 사용하여 두 테이블을 연결한 후에는 관계 세부 정보에 대해 걱정하거나 가져오기 전에 단일 테이블로 평면화할 필요 없이 단일 테이블인 것처럼 두 테이블의 데이터로 작업할 수 있습니다. 대부분의 상황에서 Power BI Desktop은 자동으로 관계를 만들 수 있으므로 사용자가 직접 관계를 만드는 작업은 필요하지 않습니다. 그러나 Power BI Desktop는 두 테이블 간의 관계를 높은 수준의 신뢰도로 판단할 수 없는 경우, 관계를 자동으로 만들지 않습니다. 이 경우 사용자가 관계를 만들어야 합니다. 
...
각 테이블에는 프로젝트 열이 있습니다. 각각의 이름은 약간 다르지만 값은 동일한 것으로 보입니다. 이 점은 중요하며, 곧 다시 다루겠습니다.
...
1. **홈 > 관계 관리**를 클릭합니다.
2. **관계 관리**에서, **새로 만들기**를 클릭하여 **관계 만들기** 대화 상자에서 테이블, 열 및 관계에 사용하려는 모든 추가 설정을 선택할 수 있습니다.
...
솔직히 말씀드리자면, 방금 이 관계를 어려운 방법으로 만들었습니다. 관계 관리 대화 상자에서 자동 검색 단추를 클릭할 수도 있었습니다. 사실 두 열의 이름이 같은 경우 자동 검색은 사용자가 데이터를 로드할 때 이미 이 작업을 완료합니다. 하지만 무엇이 문제일까요?
...
우선 순위에 따라 시간을 합하는 경우, Power BI Desktop은 CompanyProject 조회 테이블에서 고유한 색상 값의 모든 인스턴스를 찾은 후  CompanyProject 테이블에서 해당 값의 모든 인스턴스를 찾고 각 고유한 값의 합계를 계산합니다.

사실 자동 검색을 사용하면 아주 쉬우며, 이렇게 할 필요조차 없을 수도 있습니다.

## <a name="understanding-additional-options"></a>추가 옵션 이해
자동 또는 수동 중 하나를 사용하여 관계를 만들면, Power BI Desktop은 테이블에서 데이터를 기반으로 하는 추가 옵션을 자동으로 구성합니다. 관계 만들기/편집 대화 상자의 가장 아래 부분에 있는 이러한 추가 관계 속성을 구성할 수 있습니다.
...
Power BI가 보고서 및 모델의 관계를 자동으로 조정하고 취급하는 방법을 관리할 수 있습니다. Power BI가 관계 옵션을 처리하는 방법을 지정하려면, Power BI Desktop에서 **파일 > 옵션 및 설정 > 옵션**을 선택하고 왼쪽 창에서 **데이터 로드**를 선택합니다. 그러면 **관계**에 대한 옵션을 볼 수 있습니다.

 ![관계 옵션](media/desktop-create-and-manage-relationships/relationships-options-01.png)

세 가지 옵션을 선택하고 사용할 수 있습니다. 

첫 번째 옵션은 *데이터 원본에서 관계 가져오기*이며, 기본적으로 선택됩니다. 선택된 경우, Power BI는  기본 키/외래 키와 같은 데이터 원본에 정의된 관계를 확인합니다. 이러한 관계가 존재하는 경우, 처음에 데이터를 로드할 때 Power BI 데이터 모델에 미러링됩니다. 이 옵션을 사용하면 직접 관계를 찾거나 정의하지 않고 신속하게 모델 사용을 시작할 수 있습니다.

두 번째 옵션은 *데이터를 새로 고칠 때 관계 업데이트 또는 삭제* 이며, 기본적으로 해제되어 있습니다. 선택하는 경우(옵션 옆의 상자를 선택하여 설정), 데이터 집합을 새로 고칠 때 Power BI는 데이터 원본 관계의 변경 내용을 확인합니다. 이러한 관계를 변경 또는 제거하는 경우, Power BI는 고유한 데이터 모델에 변경 사항을 미러링하여 관계가 일치하도록 업데이트 또는 삭제합니다.

> [!WARNING]
> 정의된 관계를 사용하는 행 수준 보안을 사용하는 경우, 두 번째 옵션인 *데이터를 새로 고칠 때 관계 업데이트 또는 삭제*를 선택하는 것은 권장되지 않습니다. RLS 설정이 의존하는 관계를 제거하는 경우, 모델의 보안성이 떨어질 수 있습니다. 

세 번째 옵션은 *데이터가 로드된 후 새 관계 자동 검색*이며, 이 문서의 앞부분에 나오는 [로드하는 동안 자동 검색](#autodetect-during-load)에서 설명합니다. 


## <a name="future-updates-to-the-data-require-a-different-cardinality"></a>향후 데이터 업데이트에 다른 카디널리티가 필요한 경우
...
모두 설정을 사용하면 Power BI Desktop이 연결된 테이블의 모든 측면을 단일 테이블처럼 처리할 수 있습니다. 그러나 Power BI Desktop이 관계의 교차 필터 방향을 '모두'로 설정할 수 없고 보고 용도로 명확한 기본값 집합도 사용 가능하도록 유지할 수 없는 경우가 있습니다. 관계 교차 필터 방향을 모두로 설정하지 않는 경우는 보통 모호해지기 때문입니다. 기본 교차 필터 설정이 적합하지 않은 경우 특정 테이블이나 모두로 설정할 수 있습니다.
...
필터 사양은 CompanyProject에서 CompanyEmployee(아래 이미지에 표시됨)로 흐르지만 CompanyEmployee까지 진행하지 않습니다. 그러나 교차 필터링 방향을 모두로 설정하면 작동합니다. 모두로 설정하면 필터 지정이 Employee까지 진행될 수 있습니다.

 ![](media/desktop-create-and-manage-relationships/candmrel_bidircrossfiltering.png)

교차 필터링 방향을 모두로 설정하면 이제 보고서가 올바르게 표시됩니다.

 ![](media/desktop-create-and-manage-relationships/candmrel_repcrossfilterbi.png)

양쪽 방향으로의 교차 필터링은 위 패턴과 같은 테이블 관계 패턴에서도 제대로 작동합니다. 일반적으로 이를 다음과 같이 스타 스키마(Star schema)라고 합니다.
...
루프가 있는 이러한 테이블 패턴이 있는 경우 교차 필터링은 모호한 관계 집합을 만들 수 있습니다. 예를 들어 테이블 X에서 필드 합계를 계산한 다음 테이블 Y의 필드를 기준으로 필터링하도록 선택하는 경우 필터가 상단 테이블을 통해 진행되어야 하는지 또는 하단 테이블을 통해 진행되어야 하는지 명확하지 않습니다. 이러한 패턴의 일반적인 예로 판매 실적 데이터가 있는 판매 테이블인 테이블 X와 예산 데이터가 될 테이블 Y를 들 수 있습니다. 그러면 가운데 테이블이 부서, 지역 등 두 테이블이 사용할 조회 테이블이 됩니다. 

활성/비활성 관계와 마찬가지로 보고서에 모호성이 생기는 경우 Power BI Desktop은 관계를 모두로 설정할 수 없도록 합니다. 여러 가지 다른 방법으로 이를 처리할 수 있으며 가장 일반적인 두 가지 방법은 다음과 같습니다.
...
경우에 따라 모델에 여러 테이블이 있고 테이블 간의 관계가 복잡할 수 있습니다. Power BI Desktop의 관계 보기는 모델의 모든 관계, 방향 및 카디널리티를 이해하기 쉽고 사용자 지정 가능한 다이어그램으로 표시합니다. 자세한 내용은 [Power BI Desktop의 관계 보기](desktop-relationship-view.md)를 참조하세요.
