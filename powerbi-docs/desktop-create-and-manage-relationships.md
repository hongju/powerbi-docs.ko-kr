...
필터 사양 흐릅니다 CompanyProject에서 companyemployee (아래 이미지에 표시 됨) 처럼 있지만 CompanyEmployee까지 진행 하지 않습니다. 그러나 교차 필터링 방향을 양쪽으로 설정하면 작동합니다. 양쪽으로 설정하면 필터 지정이 Employee까지 진행될 수 있습니다.
