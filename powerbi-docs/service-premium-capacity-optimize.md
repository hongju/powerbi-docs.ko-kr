...
- 양방향 필터링을 허용해야 하는 확실한 이유가 없는 한 단일 방향 관계를 구성하여 모델 쿼리 성능을 개선할 수 있습니다. 관계를 모델링을 가정 (이 경우) 하는 경우 참조 무결성이 적용 되도록 구성 해야 하는 또한 데이터 원본 쿼리를 더 효율적으로 내부 조인 (대신 outer join) 사용 하 여 발생 합니다.
