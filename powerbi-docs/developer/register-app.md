...
5. **리디렉션 URL** 및 **홈 페이지 URL**에 대한 값을 입력합니다. **리디렉션 URL**은 모든 유효한 URL과 함께 작동합니다.
    ...
    애플리케이션 형식으로 **서버 쪽 웹앱**을 선택한 경우에만 **홈 페이지 URL**을 사용할 수 있습니다.
...
애플리케이션을 등록하면 **클라이언트 ID**가 수신됩니다.  **클라이언트 ID**는 애플리케이션이 사용자를 식별하도록 사용자에게 권한을 요청합니다.
...
5. **애플리케이션 ID**가 GUID로 나열됩니다. 이것은 애플리케이션에 대한 클라이언트 ID입니다.
