---
title: "US Government용 고객의 응용 프로그램에 Power BI 콘텐츠 포함"
...
---
# <a name="embed-a-power-bi-dashboard-tile-or-report-into-your-application-for-us-government"></a>US Government용 응용 프로그램에 Power BI 대시보드, 타일 또는 보고서 포함
...
공용 클라우드와 달리 US Government 클라우드에는 세 개의 범주 섹션이 있습니다.
...
이 연습을 시작하려면 **Power BI US Government 계정** 계정이 필요합니다. 계정을 설정하지 않았다면 [정부 Power BI 계정에 등록](../service-govus-signup.md)하면 됩니다.
...
REST API 호출을 실행하려면 Azure AD를 사용해 응용 프로그램을 등록해야 합니다. 자세한 내용은 [Power BI 콘텐츠를 포함하려면 Azure AD 앱 등록](register-app.md)을 참조하세요. 사용자가 속한 정부의 형식에 따라 세 가지 범주 섹션이 있으므로 응용 프로그램을 등록할 세 개의 고유 URL이 있습니다.
...
응용 프로그램 내에서 먼저 Azure AD에서 **액세스 토큰**을 가져와야 Power BI REST API로 호출할 수 있습니다. 자세한 내용은 [사용자를 인증하고 Power BI 앱에 대한 Azure AD 액세스 토큰 가져오기](get-azuread-access-token.md)를 참조하세요. 사용자가 속한 정부의 형식에 따라 세 가지 범주 섹션이 있으므로 응용 프로그램의 액세스 토큰을 가져올 두 개의 고유 URL이 있습니다.

* GCC(정부 커뮤니티 클라우드) - login.microsoftonline.com

* DoDCON(군용 계약업체) - login.microsoftonline.us

* DoD(군대) - login.microsoftonline.us
