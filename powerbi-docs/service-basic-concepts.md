...
-  *앱 작업 영역*은 동료들과 함께 콘텐츠를 공동 작업하고 공유하는 데 사용됩니다. 또한 조직을 위한 앱을 만들어 게시하고 관리하는 공간이기도 합니다. Power BI 앱을 구성할 콘텐츠를 위한 준비 영역이자 컨테이너라고 생각하면 됩니다. 앱 작업 영역에 동료를 추가하고 대시보드, 보고서, 통합 문서 및 데이터 세트에 대한 공동 작업을 수행할 수 있습니다. 모든 앱 작업 영역 구성원에는 Power BI Pro 라이선스가 필요합니다. 앱 소비자를 앱에 액세스할 수 있는 동료에 게 Pro 라이선스가 반드시 필요 합니다. 자세한 내용은 합니다 [새 작업 영역](service-create-the-new-workspaces.md)합니다.  

일반적으로 공유 하는 방법에 대 한 자세한 내용은 사용 하 여 시작 [회사 대시보드를 공유 하는 방법을](service-how-to-collaborate-distribute-dashboards-reports.md)합니다.
...
데이터 세트는 *작업 영역*과 연결되며, 단일 데이터 세트는 여러 작업 영역의 일부가 될 수 있습니다. 작업 영역을 열면 **데이터 세트** 탭 아래에 연결된 데이터 세트가 나열됩니다. 나열된 각 데이터 세트는 OneDrive의 Excel 통합 문서, 온-프레미스 SSAS 표 형식 데이터 세트, Salesforce 데이터 세트 등 데이터의 단일 원본을 나타냅니다. 여러 가지 다양한 데이터 소스가 지원되며 계속해서 새로 추가할 예정입니다. 목록은 [Power BI를 사용 하 여 사용할 수 있는 데이터 집합 형식](service-get-data.md)합니다.
...
하나의 작업 영역 구성원에 의해 추가 된 데이터 집합 사용 하 여 다른 작업 영역 구성원에 게 사용할 수는 *관리자*를 *멤버*, 또는 *참가자* 역할입니다.
...
Power BI 보고서는 꺽은선형 차트, 맵 및 트리맵과 같은 하나 이상의 시각화가 있는 페이지입니다. 시각화는 **_시각적 개체_** 라고도 합니다. 보고서의 모든 시각화는 단일 데이터 세트에서 제공합니다. Power BI 내에서 처음부터 보고서를 만들, 동료와 공유 하는 대시보드를 사용 하 여 가져올 수도 있고, Power BI, Excel, Power BI Desktop, 데이터베이스 및 SaaS 응용 프로그램에서 데이터 집합에 연결할 때 해당 만들 수 있습니다.  예를 들어 파워 뷰 시트가 포함된 Excel 통합 문서에 연결하면 Power BI에서 해당 시트를 기반으로 보고서를 만듭니다. 또한 SaaS 애플리케이션에 연결하면 Power BI에서 미리 작성된 보고서를 가져옵니다.

보고서를 보고 상호 작용하는 다음 두 가지 모드가 있습니다. *읽기용 보기* 하 고 *편집용 보기*합니다. 보고서를 열면 읽기용 보기에서 열립니다. 편집 권한이 있을 경우 표시 **보고서 편집** 왼쪽 위 모서리에서 편집용 보기로 보고서를 볼 수 있습니다.  보고서 작업 영역을 사용 하 여 모든 사람 인지를 *관리자*, *멤버*, 또는 *참가자* 역할을 편집할 수 있습니다. 디자인 모든 탐색에 액세스할 수 있는, 해당 보고서에 대 한 빌드 및 공유 기능 편집을 확인 합니다. 보고서를 공유 하는 사람들 탐색 하 고 읽기용 보기에서 보고서와 상호 작용할 수 있습니다.   

작업 영역을 열면 **보고서** 탭 아래에 연결된 보고서가 나열됩니다. 나열된 각 보고서는 단 하나의 기본 데이터 세트를 기반으로 하는 하나 이상의 시각화 페이지를 나타냅니다. 보고서를 열려면 보고서를 선택합니다.

앱을 열면 대시보드가 제공 됩니다.  기본 보고서에 액세스하려면 보고서에서 고정한 대시보드 타일(나중에 타일 추가)을 선택합니다. 보고서에서 모든 타일이 고정된 것이 아니므로 보고서를 찾으려면 타일 몇 개를 클릭해야 할 수 있다는 사실을 참고하세요.

기본적으로 보고서는 읽기용 보기로 열립니다.  **보고서 편집**을 선택하여 필요한 권한이 있는 경우 편집용 보기에서 엽니다.

아래 예제에서는 "영업 및 마케팅" 앱 작업 영역을 선택 하 고에 대 한 탭을 클릭 **보고서**합니다.
...
* 단일 작업 영역에 포함되어 있습니다.
* 해당 작업 영역 내에서 여러 대시보드에 연결할 수 있습니다. 타일을 고정할 보고서는 여러 대시보드에 나타날 수 있습니다.
* 하나의 데이터 세트의 데이터를 사용하여 만들 수 있습니다. Power BI Desktop 보고서에서 단일 데이터 집합에 둘 이상의 데이터 소스를 결합 하 고 해당 보고서를 Power BI로 가져올 수 있습니다.

  ![보고서 다이어그램](media/service-basic-concepts/drawing3new.png)

### <a name="dig-deeper"></a>심층 분석
- [데이터 집합을 가져와 Power BI 서비스에서 보고서를 만들려면](service-report-create-new.md)
- [Power BI 모바일 앱에 대 한 보고서를 최적화 합니다.](desktop-create-phone-report.md)
...
### <a name="dig-deeper"></a>심층 분석
* [빈 대시보드를 만들고 일부 데이터를 가져오고 다음](service-dashboard-create.md) 합니다.
