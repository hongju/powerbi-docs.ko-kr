...
3. **Power BI** 리본에서 **핀** 단추를 클릭하여 **대시보드에 고정**대화 상자를 표시합니다. Power BI에 로그인되어 있지 않은 경우 로그인하라는 메시지가 표시됩니다. **작업 영역** 드롭다운 목록에서 작업 영역을 선택합니다. 고유한 대시보드에 고정하려면 **내 작업 영역**을 선택하도록 확인합니다. 그룹 작업 영역에서 대시보드를 고정하려면 드롭다운 목록에서 그룹을 선택합니다.
