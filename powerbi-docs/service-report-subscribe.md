...
* 이메일 구독을 설정할 때 구독 작업이 시작되는 시점과 이메일을 보내는 시간 사이의 지연이 있다는 점을 유의하세요.  작업이 시작되는 시점과 이후 이메일이 전송되는 시점 사이의 지연 시간을 최소화하려면 예약된 데이터 새로 고침을 위해 이메일 구독이 실행되도록 예약된 시간과 다른 시간을 설정하는 것이 좋습니다.
