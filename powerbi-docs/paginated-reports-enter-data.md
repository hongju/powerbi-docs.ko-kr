...
1. **보고서 데이터** 창에서 **새로 만들기** > **데이터 집합**을 선택합니다.

    ![보고서 작성기 새 데이터 세트](media/paginated-reports-enter-data/paginated-new-dataset.png)

1. **데이터 집합 속성** 대화 상자에서 **내 보고서에 포함된 데이터 집합 사용**을 선택합니다.
...
1. **데이터 집합 속성** 대화 상자로 돌아가서 **쿼리 디자이너**를 선택합니다.
...
12. **데이터 집합 속성** 대화 상자에서 **확인**을 선택합니다.
...
이제 페이지를 매긴 보고서가 미리보기의 Power BI 서비스에서 지원되므로, 페이지를 매긴 보고서를 프리미엄 용량으로 업로드할 수 있습니다. 자세한 정보는 [페이지를 매긴 보고서 업로드](paginated-reports-save-to-power-bi-service.md#upload-a-paginated-report)를 참조하세요.
