...
Power BI Desktop의 쿼리 편집기를 사용하여 데이터를 로드하는 경우 최상의 추측 데이터 형식 검색을 수행합니다. 수식을 사용할 경우 열의 데이터 형식 설정이 유지되지 않는 경우도 있습니다. 다음 작업을 수행한 후 열의 데이터 형식이 올바른지 확인 합니다.  쿼리 탭에 처음 데이터 로드, 첫 행을 머리글로 사용, 열 추가, 그룹화 기준, 병합, 추가 작업을 수행한 이후 및 처음 데이터 로드를 누르기 이전.
...
Power BI Desktop에서 쿼리 편집기의 탐색기, 쿼리 중 하나를 마우스 오른쪽 단추로 클릭할 때 "참조"에 대 한 옵션에서는 사용할 수 있습니다. 다음과 같은 이유로 유용합니다.
...
기본적으로 모든 쿼리는 Excel 워크시트 또는 데이터 모델(또는 둘 다)로 로드됩니다. 일부 쿼리는 중간 단계 이며 최종 사용자에 게 없습니다. 위에서 언급한 대로 쿼리를 참조할 때가 바로 이러한 경우입니다. 탐색 창에서 쿼리를 마우스 오른쪽 단추로 클릭 하 고 "로드 사용" 옵션을 설정/해제 하 여 쿼리 로드 동작을 제어할 수 있습니다. 때 *로드 사용* 확인 표시가 없는 옆에 쿼리는 쿼리 탭에서 계속 제공 하 고 다른 쿼리와 함께 사용할 수 있습니다. 이 기능은 특히 병합, 추가 및 참조 변환과 조합할 때 유용합니다. 그러나 쿼리 결과가 데이터 모델에 로드되지 않으므로 쿼리가 보고서 필드 목록 또는 데이터 모델을 복잡하게 만들지 않습니다. 

## <a name="scatter-charts-need-a-point-identifier"></a>지점 식별자가 필요한 분산형 차트
온도와 시간으로 구성된 간단한 테이블을 예로 들고 판독값이 사용되었습니다. 이 테이블을 분산형 차트에서 직접 그리는 경우 Power BI는 모든 값을 단일 요소로 집계합니다. 개별 데이터 요소를 표시 하려면 추가 해야 필드를 필드에 세부 정보 버킷에 합니다. Power BI Desktop에서 이 작업을 간단하게 수행하려면 쿼리 탭의 “열 추가” 리본에서 “인덱스 열 추가” 옵션을 사용하면 됩니다. 
...
Power BI에서 차트 축이나 슬라이서 또는 필터에 범주(문자열) 값을 사용하는 경우 기본 순서는 사전순입니다. 이 순서를 재정의 해야 할 경우 예를 들어 주 또는 월, 일 등을 위해 다음 알려줄 수 있습니다 다른 열별로 정렬 하려면 Power BI Desktop. 자세한 내용은 참조 하세요 [Power BI Desktop의 열 기준 정렬](desktop-sort-by-column.md)합니다.
...
필드를 올바르게 지오코딩하는 또 다른 방법은 데이터 필드에서 데이터 범주를 설정하는 것입니다. Power BI Desktop에서 원하는 테이블을 선택 하 고 고급 리본으로 이동 다음 데이터 범주 주소, 도시, 대륙, 국가/지역, 국가, 우편 번호, 상태 또는 시/도를 설정 합니다. 이러한 데이터 범주는 Bing에서 데이터를 올바르게 인코드하는 데 도움이 됩니다. 자세히 알아보려면 [Power BI Desktop의 데이터 분류](desktop-data-categorization.md)를 참조하세요.
...
사용자 하나의 시각적 개체에 데이터 요소를 선택 하는 경우 되도록 시각적 개체가 함께 연결 되 면 지원 브러시-브러시는 히스토그램을 정의 보고서 페이지의 다른 시각적 개체 강조 표시 하거나 선택한 데이터 요소와 관련 된 데이터 요소를 필터링 합니다. 쿼리 시간에 데이터를 조작하므로 테이블 간의 관계를 만들고 히스토그램의 버킷과 관련되는 세부 항목을 알아야 하며 그 반대의 경우에도 마찬가지입니다.
