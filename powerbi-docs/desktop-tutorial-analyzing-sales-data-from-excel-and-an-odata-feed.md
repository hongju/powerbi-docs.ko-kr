...
1. 오른쪽의 **필드** 창에 있는 **주문**에서 **수량** 필드를 선택하거나 캔버스의 빈 공간으로 끌어옵니다. 그러면 주문한 모든 제품의 총 수량을 표시하는 누적 세로 막대형 차트가 만들어집니다. 
   
2. **필드** 창의 **제품**에서 **ProductName**을 선택하거나 차트로 끌어오면 주문한 각 제품의 수량이 표시됩니다. 
   
3. 가장 많은 주문부터 가장 적은 주문 순서로 제품을 정렬하려면 시각화의 오른쪽 위에 있는 **추가 옵션** 줄임표(**...**)를 선택한 다음, **수량순 정렬**을 선택합니다.
...
1. 캔버스에서 아무것도 선택하지 않은 상태로 **필드** 창의 **주문**에서 **LineTotal**을 선택하거나 캔버스의 빈 공간으로 끌어옵니다. 누적 세로 막대형 차트에 모든 주문의 총 금액이 표시됩니다. 
   
2. 차트를 선택한 상태에서 **주문**에서 **OrderDate**를 선택하거나 차트로 끌어옵니다. 이제 차트에 각 주문 날짜의 라인 합계가 표시됩니다. 
...
1. 캔버스에서 아무것도 선택하지 않은 상태로 **필드** 창의 **주문**에서 **ShipCountry**를 선택하거나 캔버스의 빈 공간으로 끌어옵니다. Power BI Desktop은 데이터가 국가 이름인지 검색하고 주문이 있던 각 국가에 대한 데이터 요소를 포함하는 맵 시각화를 자동으로 만듭니다. 
