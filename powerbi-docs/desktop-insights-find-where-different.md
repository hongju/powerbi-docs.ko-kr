...
이 예에서 *총 판매액*과 같이 단순 가산 측정값의 경우 비교는 절대값이 아닌 상대값에 기반하여 이루어집니다. 따라서 투어링 바이크의 판매가 모든 범주의 전체 판매보다 낮지만, 기본적으로 시각적 개체는 이중축을 사용하여 다른 국가 전체에서 투어링 바이크와 모든 범주의 바이크에 대한 판매 비율 간에 비교할 수 있습니다.  시각적 개체 아래의 토글을 전환하면 두 값을 같은 축에 표시할 수 있어 절대 값을 쉽게 비교할 수 있습니다(다음 이미지 참조).    
...
보고서가 읽기 또는 편집 모드인 경우 정보를 사용할 수 있습니다. 그러면 데이터를 분석하고 시각적 개체를 만드는 데 유용하게 사용되므로 보고서에 쉽게 추가할 수 있습니다.
...
*다르다는* 것이 무엇을 의미하는지 궁금할 것입니다. 예를 들어 미국과 캐나다 간 전반적인 판매 분할이 다음과 같다고 하겠습니다.
