...
이 예에서 *총 판매액*과 같이 단순 가산 측정값의 경우 비교는 절대값이 아닌 상대값에 기반하여 이루어집니다. 따라서 Touring Bikes에 대 한 판매는 모든 범주에 대 한 전체 판매 보다 낮은, 기본적으로 시각적 개체 사용 이중 축 bikes의 모든 범주 및 Touring Bikes에 대 한 다른 국가에서 판매 비율 간의 비교를 허용 하도록 합니다.  시각적 개체 아래의 토글을 전환하면 두 값을 같은 축에 표시할 수 있어 절대 값을 쉽게 비교할 수 있습니다(다음 이미지 참조).    
...
사용자가 가능성이 하는지 궁금해 하 *다른* 것을 의미 합니다. 예를 들어 미국과 캐나다 간 전반적인 판매 분할이 다음과 같다고 하겠습니다.
...
따라서 가산적 측정값의 경우 이전 예에서 *판매*와 같이 세로 막대형 차트와 꺾은선형 차트가 사용되며, 상대값을 쉽게 비교할 수 있도록 적절한 크기의 이중축을 사용합니다. 열은 필터가 적용된 값을 보여 주며 선은 전체 값을 보여 줍니다(일반적으로 열 축은 왼쪽, 선 축은 오른쪽). 선을 사용 하 여 표시 됩니다는 *단계별* 회색으로 채워진 점선으로 된 스타일입니다. 이전 예를 들어 열 축 최대값이 4이 고 줄 축 최대값은 20 이며, 하는 경우 다음 수 쉽게 필터링 하 여 전체 값에 대 한 미국 및 캐나다 간의 상대 값을 비교 합니다. 
...
다양한 열과 이러한 각 열의 값을 자세히 살펴본 후 가장 큰 차이를 보여 주는 값 집합이 선택됩니다. 그런 다음, 이해하기 쉽도록 이러한 값은 열별로 그룹화되어 산출되며, 열에는 차이가 가장 큰 값이 목록의 처음에 표시됩니다. 세 가지 값으로 최대 열당 표시 되지만 작은 경우 큰 효과 포함 하는 세 대 보다 적은 값 또는 일부 값은 다른 항목 보다 훨씬 더 강력한 경우 표시 될 수 있습니다. 
