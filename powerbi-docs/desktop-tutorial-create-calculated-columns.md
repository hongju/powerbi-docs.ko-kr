...
계산 열과 [측정값](desktop-tutorial-create-measures.md)은 모두 DAX 수식을 기반으로 한다는 점에서 비슷하지만 사용하는 방식이 다릅니다. 측정값은 다른 필드를 기준으로 결과 값을 계산하기 위해 시각화의 **값** 영역에 주로 사용됩니다. 계산 열은 시각화의 행, 축, 범례 및 그룹 영역에서 새 **필드**로 사용합니다.
