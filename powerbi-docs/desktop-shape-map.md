...
* 현재는 **범례** 분류가 제대로 작동하려면 **값** 버킷이 설정되어 있어야 합니다.
* **도형 맵**의 최종 릴리스 버전에는 현재 선택된 맵의 맵 키를 보여 주는 사용자 인터페이스가 포함되며(최종 릴리스에 대한 날짜 집합은 없지만, **도형 맵**은 미리 보기에 있음), 이 미리 보기에서 이 문서의 다음 **지역 키** 섹션에 있는 테이블의 맵 지역 키를 참조할 수 있습니다.
