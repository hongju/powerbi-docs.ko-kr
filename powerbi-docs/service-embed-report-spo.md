...
* SharePoint Online용 Power BI(미리 보기) 웹 파트는 [최신 페이지](https://support.office.com/article/Allow-or-prevent-creation-of-modern-site-pages-by-end-users-c41d9cc8-c5c0-46b4-8b87-ea66abc6e63b)가 필요합니다.

## <a name="embed-your-report"></a>보고서 포함
SharePoint Online에 보고서를 포함하려면, 먼저 보고서의 URL을 가져온 다음에 SharePoint Online 내의 새로운 Power BI(미리 보기) 웹 파트로 해당 URL을 사용합니다.

### <a name="get-a-url-to-your-report"></a>URL를 보고서로 가져오기
1. Power BI 서비스에서 보고서를 봅니다.
2. **파일** 메뉴 항목을 선택합니다.
3. **SharePoint Online에 포함(미리 보기)**를 선택합니다.
...
2. **+**를 선택하고 **Power BI(미리 보기)** 웹 파트를 선택합니다.
...
2. 보고서에 액세스해야 하는 사용자와 대시보드를 공유합니다. 자세한 내용은 [동료 및 다른 사용자와 대시보드 공유](service-share-dashboards.md)를 참조하세요.
    ...
    보안 그룹에 대시보드를 공유하는 경우 보고서에 대한 액세스를 가진 사용자의 경우 한 번 이상 Power BI에 로그인해야 합니다.
...
Power BI(미리 보기) 웹 파트 내에서 보고서가 로드되지 않고 다음과 같은 메시지가 표시될 수 있습니다.
...
* SharePoint Online용 Power BI(미리 보기) 웹 파트는 화면 판독기 또는 키보드 탐색을 지원하지 않습니다.
...
* Power BI(미리 보기) 웹 파트를 추가할 때 로그인하라는 메시지가 표시되지만 로그인할 수 없습니다. 이 경우 다음 중 하나와 비슷한 오류 메시지가 나타납니다.
  
  * *We need you to sign in again.(다시 로그인해야 합니다.) Once you do, we'll bring you back here.(로그인하면 다시 접속할 수 있습니다.)*
  * *An error occurred, please try logging out and back in and then revisiting this page.(오류가 발생했습니다. 로그아웃했다가 다시 로그인한 후 이 페이지를 다시 방문하세요.)*
  
  이 문제를 해결하려면 이 문서 앞부분의 **요구 사항** 섹션에 설명된 대로 테넌트를 First release for everyone(모두를 위한 첫 번째 릴리스)에 설정합니다. SSO(Single Sign On)를 사용하기 위해 Power BI 웹 파트는 SharePoint에서 First release for everyone(모두를 위한 첫 번째 릴리스)을 선택했을 때만 사용할 수 있는 인증 API를 사용합니다.
...
[Office 365에서 표준 또는 첫 번째 릴리스 옵션 설정](https://support.office.com/article/Set-up-the-Standard-or-First-Release-options-in-Office-365-3B3ADFA4-1777-4FF0-B606-FB8732101F47)  
