...
X축을 따라 숫자 또는 범주 데이터를 그릴 수도 있다는 점을 제외하면 점 그림 차트는 거품형 차트 및 분산형 차트와 비슷합니다.
...
1. 아이콘 ![노란색 더하기 아이콘의 스크린샷](media/power-bi-visualization-scatter/power-bi-yellow-plus-icon.png) 을 선택하여 빈 보고서 페이지를 만듭니다.

1. **필드** 창에서 다음 필드를 선택합니다.
...
1. 거품형 차트에 표시할 데이터 요소의 수를 설정하려면 **시각화** 창의 **서식** 섹션에서 **일반**을 확장하고 **데이터 볼륨**을 조정합니다.
    ...
    ![서식 아이콘 및 일반 드롭다운이 있고 데이터 볼륨 옵션이 호출된 시각화 창의 스크린샷](media/power-bi-visualization-scatter/pbi_scatter_data_volume.png)
...
1. [시각화 색, 레이블, 제목, 배경 등의 서식을 지정](service-getting-started-with-color-formatting-and-axis-properties.md)할 수 있습니다.
    ...
    ![표식 모양 옵션이 호출된 셰이프 드롭다운의 스크린샷](media/power-bi-visualization-scatter/pbi_scatter_marker.png)
...
![하나의 데이터 요소가 있는 분산형 차트의 스크린샷](media/power-bi-visualization-scatter/pbi_scatter_tshoot1.png)

**자세히** 영역에 필드를 추가하여 Power BI에 값을 그룹화하는 방법을 알립니다. 필드는 그리려는 각 지점에 고유해야 합니다. 단순 행 번호 또는 ID 필드가 유효합니다.

![자세히 웰에 RowNum이 추가된 분산형 차트의 스크린샷](media/power-bi-visualization-scatter/pbi_scatter_tshoot.png)
