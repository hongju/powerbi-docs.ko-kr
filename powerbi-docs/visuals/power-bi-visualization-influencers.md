...
8. **확인란**: **영향 요인에 해당하는 값만 표시** 확인란입니다.

## <a name="create-a-key-influencers-visual"></a>주요 영향 요인 시각적 개체 만들기 
 
먼저 이 비디오를 시청하여 주요 영향 요인 시각적 개체를 만드는 방법을 알아본 후에 다음 단계를 수행하여 만들어 보세요. 
...
## <a name="binned-continuous-key-influencers"></a>범주화된 연속형 주요 인플루언서
...
상관 관계 테스트를 실행하여 인플루언서가 대상과 관련해서 얼마나 선형인지를 확인합니다. 대상이 연속형이면 Perasons 상관 관계를 실행하고, 대상이 범주이면 Point Biserial 상관 관계 테스트를 실행합니다. 관계가 충분히 선형이 아님을 발견하면 감독된 범주화를 수행하고 최대 5개의 bin을 생성합니다. 가장 적합한 bin을 파악하기 위해 분석되는 대상과 설명 요소 간의 관계를 확인하는 감독된 범주화 방법을 사용합니다.
...
주요 인플루언서 시각적 개체에는 다음과 같은 몇 가지 제한 사항이 있습니다.
...
이 예제에서는 데이터를 피벗하여 브라우저, 모바일 및 태블릿의 새 열을 만들었습니다(데이터를 피벗한 후 모델링 뷰에서 관계를 삭제하고 다시 만들어야 함). 이제 **설명 기준**에서 이러한 특정 디바이스를 사용할 수 있습니다. 모든 디바이스가 인플루언서임이 밝혀졌으며, 브라우저는 고객 점수에 가장 큰 영향을 줍니다.
