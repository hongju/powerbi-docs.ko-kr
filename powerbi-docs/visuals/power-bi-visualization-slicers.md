...
3. 새 슬라이서를 선택한 상태로 필드 창에서 **구역** > **DM**을 선택하여 슬라이서를 채웁니다. 새 슬라이서는 이름 앞에 선택 상자가 있는 목록입니다. 
...
6. 또는 페인트 롤러 아이콘을 선택하여 슬라이서의 서식을 지정합니다. 여기에 모두 설명하기에는 옵션이 너무 많습니다. 실험을 통해 자신에게 적합한 슬라이서를 만드세요. 아래 예제에서 첫 번째 슬라이서는 가로 방향을 사용하고 항목에 색이 지정된 배경을 사용합니다. 두 번째 슬라이서는 세로 방향으로 유지되었고, 대신 보다 표준적인 모양을 위해 텍스트에 색을 사용합니다.

   ![새 슬라이서](media/power-bi-visualization-slicers/power-bi-filter-examples.png)
>[!TIP]
>목록 슬라이서 항목은 기본적으로 오름차순으로 정렬됩니다. 정렬 순서를 내림차순으로 바꾸려면 슬라이서의 오른쪽 위에 있는 줄임표( **...** )를 선택하고 **내림차순 정열**을 선택합니다.

**날짜 범위별로 데이터를 필터링하는 새 슬라이서를 만들려면**

1. 캔버스에 아무것도 선택되지 않은 상태에서 필드 창에서 **매장**을 드롭다운하고 **개장 날짜**를 시각화 창의 **값** 웰로 끌어 놓아 새 시각화를 만듭니다.
...
5. 슬라이서 형식을 변경하려면 슬라이서를 선택한 상태로 슬라이서의 오른쪽 위 영역을 마우스로 가리키고, 표시되는 캐럿을 드롭다운하고, **목록** 또는 **이전**과 같은 다른 옵션 중 하나를 선택합니다. 슬라이서 모양 및 선택 옵션이 어떻게 변경되는지 확인합니다. 
...
1. **구역 월별 판매량** 페이지의 구역 관리자 슬라이서가 선택된 상태에서
...
현재 보고서에서 **구역 월별 판매량** 페이지에도 **구역 관리자** 슬라이서가 있지만, 이 슬라이서를 **개요** 페이지에도 포함하려면 어떻게 해야 할까요? **신규 매장** 페이지에도 슬라이서가 있지만 **매장 이름** 정보만 제공합니다. **슬라이서 동기화** 창을 사용하여 **구역 관리자** 슬라이서를 이러한 페이지에 동기화할 수 있으므로 한 페이지에서 슬라이서를 선택하면 세 페이지 모두의 시각화에 영향을 줍니다.

1. **개요** 메뉴에서 Power BI Desktop의 **슬라이서 동기화**를 선택합니다.

    ![슬라이서 동기화](media/power-bi-visualization-slicers/power-bi-slicer-view-sync.png)

1.  **슬라이서 동기화** 창은 **필터** 창과 **시각화** 창 사이에 표시됩니다.  

    ![슬라이서 동기화](media/power-bi-visualization-slicers/power-bi-slicer-sync-pane.png)

1. **구역 월별 판매량** 페이지에서 **구역 관리자** 슬라이서를 선택합니다. 
    
    ![슬라이서 동기화](media/power-bi-visualization-slicers/9-sync-slicers.png)
    
3. **동기화** 열에서 **신규 매장** 페이지 및 **개요** 페이지를 선택하여 **구역 월별 판매량** 슬라이서를 해당 페이지에 동기화합니다. 
    
3. **표시** 열에서 **신규 매장** 페이지와 **개요** 페이지를 선택합니다. 이제 **슬라이서 동기화** 창이 아래 이미지와 같이 표시됩니다.

    ![슬라이서 동기화](media/power-bi-visualization-slicers/power-bi-sync-slicer-finished.png)

1. 슬라이서를 동기화하여 다른 페이지에 표시되도록 설정한 결과를 관찰합니다. **구역 월별 판매량** 페이지의 **구역 관리자** 슬라이서는 이제 **개요** 페이지의 슬라이서와 동일한 선택 항목을 표시합니다. **신규 매장** 페이지에서 **구역 관리자** 슬라이서의 선택 항목은 **매장 이름** 슬라이서에서 사용 가능한 선택 항목에 영향을 줍니다. 
...
1. 아무 페이지에서나 **구역 관리자** 슬라이서를 선택한 상태에서 **시각화** 창에서 **서식** 아이콘 ![서식 아이콘](media/power-bi-visualization-slicers/power-bi-paintroller.png)을 선택하여 서식 컨트롤을 표시합니다. 
...
**제목**은 기본적으로 **설정**되어 있으므로 슬라이서 위쪽에 데이터 필드 이름이 표시됩니다. 
1. 제목 텍스트의 서식을 지정하여 **글꼴색**을 빨강으로, **텍스트 크기**를 14pt로, **정렬**을 중앙으로, **글꼴 패밀리**를 Arial Black으로 설정합니다. 
