...
새 슬라이서를 만들려면 슬라이서 아이콘을 선택 하 수 필터링 할 데이터 필드를 선택 합니다 (으로 끌어 또는 합니다 **필터** 상자에 **시각화** 창), 또는 선택 하거나 먼저 데이터 필드를 끕니다 시각화를 만들고 시각화를 슬라이서로 변환할 슬라이서 아이콘을 선택 합니다. 데이터 형식에 따라 다양한 효과 및 옵션이 포함된 서로 다른 형식의 슬라이서가 생성됩니다. 
...
**구역 관리자 별로 데이터를 필터링 하 여 새 슬라이서를 만들려면**

1. Power BI Desktop 또는 Power BI 서비스에서 엽니다는 [소매점 분석 샘플](../sample-retail-analysis.md)합니다. (Power BI 서비스에서는 왼쪽 위에 있는 **보고서 편집**을 선택합니다.)
2. 개요 페이지의 선택에 캔버스에서 선택한 아무 것도 없으면 합니다 **슬라이서** 아이콘 ![슬라이서 아이콘](media/power-bi-visualization-slicers/slicer-icon.png) 에 **시각화** 새 슬라이서를 만들려면 창입니다. 
3. 새 슬라이서를 선택 하면 필드 창에서 선택 **구역** > **DM** 여 슬라이서를 채웁니다. 새 슬라이서는 이름 앞에 선택 상자가 있는 목록입니다. 
...
6. 또는 슬라이서의 서식을 지정 하려면 페인트 롤러 아이콘을 선택 합니다. 가지를 모두 여기서 설명-따라서 실험 만들 슬라이서는 작동 하기 단순히 너무 많은 옵션이 있습니다. 아래 예제에서 공간을 절약 하기 위해 드롭다운으로 변환 된 첫 번째 슬라이서 및 "모두 선택" 하는 옵션이 추가 되었습니다.  두 번째 슬라이서 색, 글꼴 크기를 사용 하 여 포맷 하 고 단일 선택을 허용 합니다.
...
1. 캔버스에서 선택한 항목 없음를 사용 하 여 드롭다운 목록 **날짜** 필드 창에 끌어서 **날짜** 하는 **값** 새 시각화를 만들려면 시각화 창에서 상자입니다.
...
5. 이제 필드 값을 변경 **날짜** 하 **MonthIndex**합니다.  생성이 **간의** 기본적으로 범위 슬라이더 슬라이서 종류 이지만 다른 슬라이서 형식 및 선택 옵션을 변경할 수 있습니다. 슬라이서 형식을 변경하려면 슬라이서를 선택한 상태로 슬라이서의 오른쪽 위 영역을 마우스로 가리키고, 표시되는 캐럿을 드롭다운하고, **목록** 또는 **이전**과 같은 다른 옵션 중 하나를 선택합니다. 슬라이서 모양 및 선택 옵션이 어떻게 변경되는지 확인합니다. 
