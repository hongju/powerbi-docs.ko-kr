...
> [!IMPORTANT]
> OneDrive에서 파일 관리를 처리하는 방법에 주의해야 합니다. OneDrive 파일을 데이터 원본으로 설정하는 경우 Power BI는 새로 고침을 수행할 때 파일의 항목 ID를 참조하여 일부 시나리오에서 문제가 발생할 수 있습니다. 마스터 파일 _A_와 해당 파일의 프로덕션 복사본 _B_가 있고 파일 B에 대해 OneDrive 새로 고을 구성하는 시나리오를 가정해 보겠습니다. 그런 다음, 파일 A를 파일 B에 _‘복사’_ 하면 복사 작업에서 이전 파일 B를 삭제하고 다른 항목 ID를 사용하여 새 파일 B를 만들어 OneDrive 새로 고침을 중단합니다. 대신 파일 B를 업로드하고 대체하여 동일한 항목 ID를 유지해야 합니다.
